...
>
> **填写顺序**：① 开头引用块（状态 / 配套 / 来源）→ ② 一结转通用约定（沿用 C-1~C-3 + 延续约定，逐条核对仍有效）→ ③ 二结转未完成需求（盘点上一周期遗留 + `UNPLANNED_ISSUES.md`）→ ④ 三结转未拍板决策 → ⑤ 四本周期痛点与方案（4.1 复查 → 4.2 方向拍板 → 4.3 范围；执行中决策/质疑回写 4.0）→ ⑥ 六变更记录每改必记。随周期演进，"沿用约定"如出现不再适用项，按红线措辞约定（一①）申请调整、获准后修订本模板同步。
>
...
> 评审时逐条决定：纳入本周期 / 转 `UNPLANNED_ISSUES.md` 留痕 / 废弃。执行中证伪的 N# 按 G11 登记质疑，裁决后按同口径流转，并在六节变更记录留痕。
...
| 日期 | 版本 | 内容 |
| ---- | ---- | ---- |
| 2026-09-15 | — | **模板自身修订**（复制为实例时删除本行）：延续约定新增 ⑦ 质疑协议（G11 交叉引用）；4.0 改名“已回写技术决策与质疑记录”并纳入质疑记录格式；4.1 证据列要求可复核定位 + 执行中证伪流转；五节降为指针节（范围唯一落点 4.3，消除双口径） |
| `<日期>` | 0.1 | 新建本文档（结转稿）：…… |