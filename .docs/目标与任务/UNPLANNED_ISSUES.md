...
| U-25 | 观察（分页正确性） | `/search/keywordSearch` 的 SQL 排序为 `ORDER BY c.create_time DESC`（**无 id tie-breaker**），而 `/feed`、`/profile` 均为 `create_time DESC, id DESC` → 同一秒创建的多条内容在页间顺序**不确定**，offset 分页下跨页可能**重复或漏项**（不稳定排序漂移）。放大 chunk 只减少跨页次数、不消除风险；且前端 `chunkedList` 的 `seen` 去重会**掩盖**该后端缺陷（helper 注释已明写"去重只兜漂移，不替代后端『页间不重不漏』的正确性"） | `content/dao/ContentDao.java:272`（单字符 `LIKE` 分支）与 `:278`（`MATCH … AGAINST` 分支）——两处 `ORDER BY c.create_time DESC` 后均无 `, c.id DESC` | `260921-prep-cleanup`/U-25（第七期 T19 窗口探索发现，2026-09-21） | 待定（留池）：修法 = 两分支补 `, c.id DESC`（与 feed/profile 同口径；改变同秒内容的确定顺序，须先拍板）；**验证手段依赖小信封**（`pageSize=1/3` 逐页比对） |
| U-26 | 代码债（测试工具健壮性） | **`tools/run_tests.py` 的 Tomcat 就绪等待硬编码 90s，余量偏紧**：同一份代码连续三轮 `test all` 的实测就绪耗时 = **62.0s / 61.0s（通过）** 与 **~87s（超时 → `phase=all exit=6` / "pytest 无汇总行" 的假失败）**——失败轮里构建成功、应用最终也已就绪，只是等待窗口用尽（`tomcat_stderr.log`：20:13:10 开始展开 war → 20:14:35 才打完 `LogUtil` 装配 → 20:14:38 判定超时），重跑即绿。根因 = 每次 `test all` 都重建 war（mtime 更新）→ Tomcat `ExpandWar` 必然**删除并重新展开** `webapps/ROOT`（8.2MB / 数百文件），Windows 磁盘上这段耗时与缓存状态强相关 | `tools/run_tests.py:76`（`START_TIMEOUT_SECONDS = 90`）；证据 = `.stage8-target/test-reports/run-20260921_200305.log`（62.0s 就绪）、`run-20260921_201227.log`（等待超时 exit 6）、`run-20260921_201554.log`（61.0s 就绪） | 日志周期 T2 窗口发现（2026-09-21） | 待定（留池）：方向 = ① 提高等待上限或改为"每 2s 探测 + 上限可配"；② 降低单次成本（部署前先清 `webapps/ROOT`，或只更新变更文件而非整包重展开）。**与 T2 改动无关**（T2 未触构建/启动链路，仅恰好在本窗口遭遇） |
