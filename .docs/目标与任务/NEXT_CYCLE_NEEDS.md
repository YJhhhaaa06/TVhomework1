...
**质疑记录**：T2 窗口 L1 仅记录 2 条（已载入任务清单 T2 执行回写，不打断）：① 日志标签 `contentId=`/`commentId=` 统一为 `id=`（保留 内容/评论 词，非对外契约）；② 常青文档 header 版本号存在 T1 未 bump 的滞后（2.16 表行后 header 仍 2.14，T1 的 6.13 无对应 header/12 行）——已随 T2 bump 至 2.17 并登记观察，留 T8 收尾统一对齐。**T3 窗口 L1 仅记录 1 条**（已载入任务清单 T3 执行回写）：域类内部日志措辞（"关注状态缓存读失败"等）统一为 SetCache 措辞，非对外契约（T2 同先例，无 L2/L3）。
