...
> 状态：**待立项（2026-09-19 建；2026-09-20 T11 窗口更新）**——第六期「杂务/准备周期」（`260919-prep-cleanup`，分支 `prep/next-phase`，T1~T9 全部完成）已归档，本档为**第七期结转稿**：二/三节承接归档周期的未关闭事项与留池项。**主线方向仍未拍板**；已完成：① 周期探查（2026-09-19）——4.1 登记 **N9~N16** 跨方向通用前置（N1~N8 已按用户指令移出）② **任务清单已建**（`NEXT_CYCLE_TASKS.md`：现 **13 个任务**——T10-A/T10-B 已完成、T11-A/T11-B/T11-C 已完成（2026-09-20）、T12~T18 清理类、**T19** 推后项）③ `UNPLANNED_ISSUES.md` 池已按 2026-09-19 评估收敛（见该档）；④ **T10-A/T10-B 已完成**（评论分页两键组 + 主楼窗口装载 + 楼中楼 K=2 + 展开接口，`refactor(cache-10a/10b)`）；⑤ **T11 已于 2026-09-20 窗口完成范围拍板并拆 T11-A/T11-B/T11-C**（接口口径 / 评论信封 + 公共 helper / 装载侧解耦），四要素已回写；⑥ **T11-A / T11-B / T11-C 均已于 2026-09-20 落地**（`refactor(cache-11a/11b/11c)`；T11-C 含 DDL 闭环 + 池 U-18 治本 + 池 U-21 处置）；⑦ **T12（池 U-14：事务边界同型未治点 2 处）已于 2026-09-20 落地**（`refactor(cache-12)`；`search` 与关注/粉丝列表装载的缓存读移出事务回调）；⑧ **T13（池 U-16：注册后自动登录缺兜底）已于 2026-09-20 落地**（`fix(user)`；`registerAndLogin` 兜底——注册成功即成功，自动登录失败返回 `token=null` 的 LoginVO，前端提示手动登录）；⑨ **R-03 已于 2026-09-20 T14 窗口拍板并落地**（① content↔comment 环**保留现状并显式登记**、② 分页信封上移 `common.model.dto` 且同形二分消除、③ `ResultMap` 按域拆方法 + 删 `dao` 包）→ **T14（包层结构清扫，治池 U-07 + U-19 + N14）已完成**（`refactor(cache-14)`；全链 exit 0 + JUnit 511/0/0/0 + pytest 145；顺带真消除 `content↔dao` / `dao↔user` 两环）；⑩ **T16（N11：Controller 基建收敛）已于 2026-09-20 落地**（HTTP 侧 4 处 `new ObjectMapper`——实测含 `LoginController` 死字段共 4 处，收敛为 `BaseServletUtil.mapper` 唯一源 + `RequestParser` 归并；删 `BaseServlet` 冗余 mapper/write* 与零调用 3 参重载；`BaseServletUtil` 去 `extends HttpServlet`；全链 exit 0 + JUnit 511/0/0/0 + pytest 145；subagent 独立 review APPROVE）。⑪ **T17 IoC 注入失败可观测（N12 前半）已于 2026-09-20 落地**（逐条 WARNING + 可配 fail-fast，JUnit 514 + pytest 145）；⑫ **T18 配置卫生（N13）已于 2026-09-20 落地**（`-Dupload.path` 外部覆盖 + `AppConfig` 带默认值读取，JUnit 522 + pytest 145）；⑬ **T19 四域分页口径统一已于 2026-09-21 落地**（feed/search/profile/follow 均为域级上限 100 + 域级信封 100、follow 由 200 降 100、公共 cap 50 与单参/两参重载删除、前端只传 `page`；JUnit 538 + pytest 161 + 全链 exit 0）。评审时按 `R-##` / `N#` / `T10`~`T19` 点单。
...
**T18（`chore`，2026-09-20 拍板并落地——配置卫生：upload.path 外部覆盖通道与 AppConfig 带默认值读取的形态）**：
外部覆盖统一通道 = **JVM 系统属性 `-Dupload.path`**（否决"只靠 `UPLOAD_PATH` 环境变量"——`AppShutDownListener` 强校验 context.xml base == upload.path，只设环境变量时 context.xml 打包值 ≠ 覆盖值必拒绝启动，即 N13"换环境不可用"实锤；否决"配置 `EnvironmentPropertySource`"——需改 Tomcat 安装目录 catalina.properties，且生产部署者环境不可控）。依据 = Tomcat 10.1 官方文档：context.xml 支持 Ant 风格 `${name:-default}` 替换、`SystemPropertySource` **始终启用**（`config/index.html` Overview + `config/systemprops.html` Property replacements）；AppConfig 同步读系统属性（优先级 环境变量 > `-Dkey` > app.properties），与 context.xml 展开**两侧同源天然一致**。实现点：`src/main/webapp/META-INF/context.xml` 的 `<PostResources base="${upload.path:-D:/data/projects/VideoPlatform/stone}"/>`（默认值与 `app.properties` **保持同步**）；`AppConfig.load()` 环境变量查不到时 fallback `System.getProperty(key)`；`AppConfig` 新增包可见带默认值重载 `get/getInt/getLong/getBoolean(key, def)`（键缺失/空 → 默认值、数值解析失败照旧抛 fail-fast；**既有 getter 语义零变化**——`getIocFailFast` 等不动）；`AppShutDownListener` 新增 `resolvePlaceholder`（**只模拟 Tomcat 展开**：sysprop > 占位符默认，不查环境变量），校验语义 = "Tomcat 实际挂载 == AppConfig 实际使用"。残余窗口（已接受）：只设 `UPLOAD_PATH` 环境变量（不设 `-D`）仍拒绝启动（与 T18 前行为一致，非回归；run_tests 靠 patch WAR context.xml 保证一致，不受影响）；文档注明覆盖 `upload.path` 推荐 `-Dupload.path`（`CATALINA_OPTS`/setenv）。落点：`webapp/META-INF/context.xml`、`config/AppConfig`、`controller/AppShutDownListener`、`AppConfigTest`、`AppShutDownListenerTest`、一次性验证脚本 `temp_script/t18_verify_upload_override.py`（详见 `常青/CURRENT_ARCHITECTURE.md` 5.1.1；验证 JUnit 522 + pytest 145 + 全链 exit 0 + 沙箱闭环 PASS 见 `NEXT_CYCLE_TASKS.md` T18 执行回写）。

**T19（`refactor(cache-19)`，2026-09-21 拍板并落地——四域分页口径统一为「域级上限 100 + 域级信封 100」）**：
① **信封取值 100**（否决原拟 200：用户判定"200 太高"、单次成本过大；亦否决评论域同款 500——内容列表 VO 更重且无需求背书）；② **不采纳"后端硬忽略 `pageSize`"**（用户主张"不让前端选、由后端决定"被拆为两层：**前端不传** = S1，已零代价满足；**后端硬忽略** = S2，其唯一增量是"参数语义更纯粹"，代价是**永久放弃三域"页间不重不漏"的自动化验证能力**——该能力对 follow/comment 是既有用例依赖，对 feed/search/profile 是**未来唯一低成本手段**，而池 U-25（search 排序缺 tie-breaker）将来只能靠小信封逐页比对抓取）；③ **follow 域同批 200 → 100**（四域同口径，避免同一列表页大小存在两套值）；④ **comment 域本轮不动**（用户："后续开任务大改"，可能含热度排序 → 见推后项 T20；其中"引入消息队列"触碰"不引入 MQ"技术红线，须在该方向立项时先拍板放开或改用进程内线程池）；⑤ **归一逻辑下沉为 request 无关纯函数** `normalizePage(Integer)` / `normalizePageSize(Integer,max,defaultSize)`，`parsePage` 与三参 `parsePageSize` 委托二者（唯一源仍是 `BaseServletUtil`）——search 的 JSON body 分支拿不到 request parameter，不抽纯函数就会造出第二份口径（与 T16"唯一源"方向一致）；无参 / 两参重载与 `DEFAULT_PAGE_SIZE_MAX`/`DEFAULT_PAGE_SIZE` 随之删除；⑥ `SearchController` 的 GET 分支改走同一归一出口（顺带消除 `page=abc` 的旧 500）。落点：`controller/BaseServletUtil`、`content/controller/{Feed,Profile,Search}Controller`、`follow/controller/FollowController`、`static/js/views/{follow,search,publish,user}.js`（详见 `常青/CURRENT_ARCHITECTURE.md` 4.2/4.3/6.17/6.18/6.19/7.2/7.3/8.1；验证 JUnit 538 + pytest 161 + 全链 exit 0 见 `NEXT_CYCLE_TASKS.md` T19 执行回写）。
...
| **四域分页口径统一**（feed / search / profile / follow） | 4.2 ② 推后项 + **用户 2026-09-21 拍板**（信封 100、不硬忽略 `pageSize`、follow 同批降 100、comment 不动） | **T19**（✅ 已完成 2026-09-21） | 域级上限/信封均 **100**（follow 由 200 降 100）；`BaseServletUtil` 归一化下沉为 `normalizePage`/`normalizePageSize` 并删公共 cap 50；前端 4 文件只传 `page`；JUnit 538 + pytest 161 + 全链 exit 0 |

**本周期明确不做（反面清单，与"纳入"同等重要）**：

| 不做 | 原因 / 去向 |
| ---- | ---- |
| ~~`feed` / `search` / `profile` 的**后端**大分页（域级上限 + 域级信封）~~ | **已随 T19 落地（2026-09-21）并从本清单移出**：四域统一为域级上限/信封 **100**（含 follow 由 200 降 100），前端只传 `page`。**仍不做**：offset→游标（属 feed 流方向） |
...
- **留池未排**：见二与 `UNPLANNED_ISSUES.md` **当前有效项 = U-11 / U-17 / U-22 / U-23 / U-24 / U-25**（`U-22` 2026-09-20 T12 窗口、`U-23` 2026-09-20 T15 窗口、`U-24`/`U-25` 2026-09-21 T19 窗口登记）；其余原池项去向均已落定（**`U-07`+`U-19`→T14 ✅ 已完成 2026-09-20**、**`U-14`→T12 ✅ 已完成 2026-09-20**、`U-16`→T13 ✅ 已完成 2026-09-20、`U-18`→T11-C ✅、`U-20`→T10、`U-21` 随 T11-C-2 关闭、`U-15` 属日志体系改造方向）。
