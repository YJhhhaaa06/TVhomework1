...
| 候选 | 内容 | 依据 | 结论 |
| ---- | ---- | ---- | ---- |
| **评论分页（后端大分页 + 前端小分页）** | **预告（本节只记意图，方案待窗口调查）**：让评论分页真正"大"起来——命中路径单次成本与评论总量**弱相关**、请求数下降。方向 = 把评论**主楼与楼中楼缓存分开**（现为 `content:comments:{id}` 整树单键），**可能涉及 `comment` 表索引调整**；前端配合"大 chunk 拉取 + 本地小批展示" | 用户 2026-09-19 澄清（后端大分页不止改前端 chunk，还含评论主楼/楼中楼缓存分开、可能动 DB 索引）；T8 的 D1=A 仅做展示层切片、命中路径仍整树反序列化（原 `U-20`）；切片接缝已收敛为 `ContentService.sliceRoots` 唯一方法 | **已落 T10（预告条目）**：范围与四要素由执行窗口调查后拍板并回写；**前置** = ① 用户重新定义 T8 红线"不动缓存装载结构" ② 若动 `comment` 表索引 → 按 G9 备份报备。**不做**：offset→游标（属 feed 流方向） |
...
> **（2026-09-19 按用户口径调整）**：候选**已拆为两个任务**——**T10 评论分页 / T11 关注·粉丝列表分页**（见 `NEXT_CYCLE_TASKS.md`），**均为预告条目**：实现形态（缓存侧窗口读 vs DB 窗口读）、是否调整 `comment` 表索引、任务内范围与四要素，一律由**执行窗口调查后拍板并回写**；本节只保留目标、已确认事实与前置条件。
