...
| ---- | ---- | ---- | ---- | ---- |
| R-03 | content ↔ comment 包层环（= 二节 R-02）怎么处置 | `260919/R-04`（= 池 `U-07`） | **待拍板（已落 T14）** | 已与池 `U-19`（分页信封上移公共包）合并为 **T14「包层结构清扫」**；**开工前须先拍板**：① 环的处置口径（拆共享组件 / 允许现状不治）② 信封上移的目标包与范围（涉 content 域 6 main + 4 测试文件 import）。多周期未动；包架构纯净化属"加新功能前的地基" |
...
**T10-A / T10-B（`refactor(cache-10a)` 起，2026-09-19 拍板——评论分页拆解：缓存窗口读 + 主楼窗口装载 + 楼中楼 K=2）**：
拍板取向：命中路径单次成本与评论总量**弱相关**（治 U-20 根因）+ 装载按需（不一次性查全库）。形态 = **A. 缓存窗口读（两键组）**——主楼 List（`content:commentRoots:{id}`，LRANGE 窗口）+ 楼中楼 Hash（`content:commentReplies:{id}`，field=主楼 id，HMGET 该页）；主楼按窗口装载（水位不足才 DB LIMIT 查询追加，复用单飞），楼中楼随该批主楼装配；**DB 窗口读 / hot_comments 打分表 / 前端去重均明确不做**（后者结构性硬伤：写热路径放大 + 分页语义破坏）。任务拆 **T10-A**（两键组 + 主楼窗口装载 + DDL，**对外契约零变化**：children 仍全量随行、缺省全量数组、pytest 评论用例零改动）/ **T10-B**（楼中楼前 **K=2 条**固定不算法 + **`reply_count` 字段**作总数 + 展开加载回复接口 + 前端大 chunk/本地小批 + 公共"分块列表"helper 治 N15，**契约变更**获批准）。DDL（G9 已定）：`comment` 加 `reply_count INT NOT NULL DEFAULT 0`（T10-A 建、T10-B 消费）+ 复合索引 `(content_id, parent_id)`（基础索引，与热度排序无关）；执行走备份闭环（改前备份 → ALTER 3306 → 备份新结构 → 重建测试库 3307）。**相关性拍板**：① T8 红线"不动缓存装载结构"由用户**放开**（本任务正是要动它）；② 热门评论排序延后（登记 **N17**，另想对策，不占 T10）；③ 热度排序正确姿势预告 = 排序参数化（`order=hot|time` + ZSet score + 候选索引 `(content_id,is_deleted,like_count)`），与 hot_comments 打分表方案的本质区别 = 静态排序一等公民、页间由构造保证、无需前端去重。落点：`content/service/CommentCache`、`content/service/ContentService`、`comment/dao/CommentDao`、`comment/service/CommentService`、`cache/CacheKeys`（详见 `NEXT_CYCLE_TASKS.md` T10-A/T10-B；验证 JUnit + pytest + G9 DDL 闭环见该档执行回写）。
...
**T11-A（`refactor(cache-11a)`，2026-09-20 拍板并落地——follow 域接口口径：后端域级信封 + 缺省反转为第一页）**：
落地取向：**信封大小的决定权从"前端传参"移到"后端域级常量"**（前端从此不出现"要多少条"的魔法数），并把缺省路径从"可拉全量的放大后门"归一为「第一页信封」。关键实现点：① `BaseServletUtil` 新增**三参重载** `parsePageSize(req, max, defaultSize)`（传了 → `min(s, max)`、未传 / 非法 → `defaultSize`），两参重载改为委托 `(req, max, DEFAULT_PAGE_SIZE)` 并抽出常量 `DEFAULT_PAGE_SIZE = 10` → **公共语义零变化**（`/feed`·`/search`·`/profile`·`/content`·`/coupon` 不受影响）；② `FollowController` 自持 `FOLLOW_PAGE_SIZE_MAX = 200`（镜像评论域 500 先例）+ `FOLLOW_PAGE_SIZE_DEFAULT = 200`，**删除 T7 的 `hasPagingParams` 双分支**（缺省=第一页信封，契约反转）；③ `FollowService` **删除两个缺省全量重载**（grep 实测无调用方，分页读成唯一入口）；④ `static/js/views/user.js` 的 sheet 接公共 `chunkedList`（请求只传 `page`、大 chunk 200 + 小批 10、本地余量内 0 请求、每次打开重建实例防串台）。**明确不做**：① 后端"硬忽略 / 删除 `pageSize` 参数"——**否决**，那会让 pytest 用小信封逐页比对"页间不重不漏"的能力失效，属用测试成本换 cap 200 已覆盖的收益；② `feed`/`search`/`profile` 的后端口径（推后 **T19**，本次只迁前端）；③ 缓存装载结构（属 T11-C）、`buildUserList` 事务边界（属 T12）。**残余窗口**：`FollowCache.getFollowerIds(long)` 在缺省重载删除后主代码已无调用方（成 follow 域死方法）——**未连带删除（越界）→ 留 T11-C 窗口评估**（L1 记录见 `NEXT_CYCLE_TASKS.md` T11-A 执行回写）。落点：`controller/BaseServletUtil`、`follow/controller/FollowController`、`follow/service/FollowService`、`static/js/views/user.js`（详见 `常青/CURRENT_ARCHITECTURE.md` 6.17；验证 JUnit 482 / pytest 145 见 `NEXT_CYCLE_TASKS.md` T11-A 执行回写）。
...
**T11-B（`refactor(cache-11b)`，2026-09-20 拍板并落地——评论域固定信封 + 公共 helper 去重/自适应 + 内容列表前端迁移）**：
落地取向：**把"信封大小"的决定权从请求参数彻底移到后端域级常量**（评论域缺省 200、上限 500 不变；`/comment/show` 与 `/comment/replies` 前端只传 `page`），并让公共 helper 承担"防重复展示"与"不写死页大小"两件事，再把 `feed`/`search`/`profile` 三处内容列表**仅前端**接入（`chunk = 50` 顶现有公共 cap，后端上限/信封归 **T19**）。关键实现点：① `CommentController` 增 `COMMENT_PAGE_SIZE_DEFAULT = 200`，两处 `parsePageSize(req, 500, 200)`（`hasPagingParams` 分支与"缺省全量数组"契约**保留**）；② `chunkedList` 增 `keyOf` + `seen`（**批内与跨 chunk 双向去重**）与"批不满时继续消费/续拉"（单次最多 10 个 chunk 防死循环），`reset()` 清 `seen`，失败时游标/去重集**不提交**；③ `chunkSize` 降级为"初始/兜底"，**以响应回显 `pageSize` 自适应**；④ `detail.js` 评论主楼与展开回复均只传 `page`；⑤ `follow.js`/`search.js`/`publish.js`/`user.js`（创作网格）四处迁移到 helper（小批 10/10/12/12），`publish.js` 的删除模式切换/删除/编辑后改为**本地条目集重渲染**（编辑走单条定向刷新），顺带消除"第 N 页重复追加第 N 页"的既有缺陷。**明确不做**：① `feed`/`search`/`profile` 的**后端**上限与域级信封（→ **T19**）；② 删评论域 `hasPagingParams`（本轮不动评论信封形状）；③ 改 `chunkedList` 缺省 `keyOf` 顺序（按拍板 `userId`→`commentId`→`id`）——**评论 VO 同时含 `userId`，故评论类调用点一律显式传 `keyOf`**（陷阱已在 helper 注释/常青 6.19/调用点三处兜住）。**残余/边界**：项目无 JS 测试层 → helper 去重与四处迁移无自动化断言（本窗口以一次性 Node 行为核验佐证）；`SearchController` 的 `pageSize` 不走公共解析（缺省 12、无上限）属既有事实，归一留 T19。落点：`comment/controller/CommentController`、`static/js/chunkedList.js`、`static/js/views/{detail,follow,search,publish,user}.js`（详见 `常青/CURRENT_ARCHITECTURE.md` 6.18/6.19 与 8.1；验证 JUnit 482 / pytest 145 见 `NEXT_CYCLE_TASKS.md` T11-B 执行回写）。
```
...
> **与已登记项的分工**：日志方向主干项 `U-15`、feed 方向事项（`R-01` 索引全量读、`U-18` 装载全量等）均随各自方向立项盘点，本节不重复；`U-14`（事务回调内缓存读）、`U-19`（信封同形二分）另有独立登记。
