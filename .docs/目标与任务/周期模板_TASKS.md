...
>
> **填写顺序**：① 开头引用块（状态 / 来源 / 明确不做）→ ② 一周期约定（G1 的 commit 前缀、G9 的 DDL 标注随周期校准）→ ③ 二任务清单理念与任务模板（沿用）→ ④ 三任务总览（按 NEEDS 4.3 拆 T1~T#）→ ⑤ 四任务详情（每任务四要素骨架）→ ⑥ 五变更记录。任务执行中按 G5/G7/G11 回写清单 / 需求文档（G11 = 质疑协议，见二节）；完成后追加"执行回写"，并在变更记录登记。
>
