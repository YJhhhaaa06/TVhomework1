...
| 编号 | 约定 | 内容 |
| ---- | ---- | ---- |
| G1 | 一任务一窗口一 commit（默认期望） | 每个任务开独立窗口，默认期望 1 个 commit；因任务内部依赖需拆多 commit 或小任务合并时，**在该任务详情标注**；commit message 按 `说明书/COMMIT_CONVENTION.md` 规范书写且**强制带任务编号**，本周期前缀 = **`<commit scope>`**（如 `fix(cache-0N)`；引用任务一律用 `T1`~`T#`，不用 commit scope） |
