...
> **执行回写（2026-09-20，第七期 T14 窗口）**
>
> * **证据复核（G11 第 (0) 项）**：四条断言全部成立。**事实修正（L1）**：四要素原文只点 `ResultMap` 的 **1** 条反向 import，实测为 **5** 条（`content.model.cache.CommentCacheDTO`、`content.model.cache.ContentCacheDTO`、`content.model.entity.ContentMedia`、`user.model.entity.User`、`admin.model.vo.AdminContentVO`——跨 content / user / admin 三域）。**新增证据**：一次性脚本 `temp_script/t14_domain_matrix.py`（gitignored）实测全仓跨域 import 矩阵——**双向包环共 9 个**（`admin↔content`、`comment↔content`、`comment↔like`、`content↔dao`、`content↔like`、`content↔upload`、`content↔user`、`dao↔user`、`ioc↔util`），其中 `content↔comment` 15 条边（comment→content 10 / content→comment 5）里**只有 5 条属"组件错位"**，其余为真业务互依。
> * **拍板结论（R-03，用户裁决 2026-09-20）**：① **环 = 口径①"允许现状不治"**——否决"搬移错位组件回 comment 域"：环**不消除**（只是边的方向重新分布），且 content→comment 由 5 条**升到 6 条**、`like→comment` +1，成本（3 类 + ~10 处 import + 常青 4 处归属描述）换来的是"归属观感"，对外行为/能力/可测性零变化；② **信封目标包 = `com.itheima.common.model.dto`**——否决 `com.itheima.model.dto`（与 `util`/`cache` 并列易被误读成"全局实体模型"）与 `com.itheima.controller`（`ProfileVO` 等 model 层反向 import controller 层，层次倒置）；③ **N14 = 按域拆方法 + 删 `dao` 包**——否决"`ResultMap` 整体搬 `content/dao`"（问题平移、且新增 2 条反向边）与"保留为泛型行读取工具"（6 处映射体需逐字段重写，是**重写**不是搬移，风险最高）。拍板依据与矩阵证据落 `.docs/temp/T14_PLAN.md`。
> * **落地（4 个动作）**：① 新增 `com/itheima/common/model/dto/PageResult.java`（**73 行**，包名与 javadoc 外主体逐字保留）；② 删除 `content/model/dto/PageResult.java`（62 行）与 `follow/model/dto/FollowPageResult.java`（79 行）——**同形二分消除**，信封唯一源落公共包；③ 删除 `dao/ResultMap.java`（**89 行**——CA 旧表记 88，实测 89，旧值即滞后），6 个行映射方法下沉为各自 DAO 的 `private static` 方法（`ContentDao` 2 / `ContentMediaDao` 1 / `CommentDao` 1 / `UserDao` 2），**20 处调用点去 `ResultMap.` 前缀**；④ import 调整 = main 8 文件（content 6：`ContentService`/`FeedService`/`ProfileService`/`FeedController`/`SearchController`/`ProfileVO`；comment 1：`CommentService`；follow 1：`FollowService`）+ test 4 文件（三个 content 测试换 import；`FollowServiceTest` 换 import **并改 9 处类型引用**）。
> * **搬移一致性核验（行为零变化的直接证据）**：一次性脚本 `temp_script/t14_verify_moved.py` 从 `git show HEAD:…/dao/ResultMap.java` 抽出 6 个方法体，与拆分后各 DAO 内同名方法体**逐行字符串比对**——6/6 全部相等（13 / 7 / 6 / 21 / 5 / 5 行）。导入顺序变动仅限 `java.time.LocalDateTime` 新增到 `ContentDao`（原 `ResultMap` 的 import）。
> * **环数变化**：`content↔dao` 与 `dao↔user` **两个环随 `dao` 包删除而真消除**（基础包不再 import 业务模型）；其余 7 个环（含 `content↔comment`）保留现状，已在 `常青/CURRENT_ARCHITECTURE.md` 4.1 显式登记（含 9 环实测清单与"为何不治"的一句话理由）——环不再"无人知晓"，而是"已登记、已接受"。
> * **明确不做**：不动 content↔comment 的共享组件归属（`CommentCacheDTO` / `CommentVO` / `CommentCache` 留在 content 域，`CommentVO` 仍归 content——与既有文档口径一致）；不改任何接口契约 / JSON 形状 / 前端；不改缓存 key / TTL / 三态 / 降级 / 打点；不改 `@WebServlet` URL / web.xml / IoC 扫描；无 DDL；无新依赖。
> * **验证（落盘证据）**：`python tools\tv.py test junit` **exit 0**、`latest.json` junit = **511 / 0 / 0 / 0**（26 个 surefire 报告逐类求和一致；**用例数与 T13 完全相同** = 纯搬移未新增/改写任何断言）；`python tools\tv.py test all` **exit 0**（`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`）。**war 内容核验**（已知坑：`tv.py` 构建不含 `clean`，删类后陈旧 `.class` 会残留）→ 先手工清掉 `.stage8-target/classes` 与 war 解包目录下的 3 个旧类再全链构建：war 内只剩 `WEB-INF/classes/com/itheima/common/model/dto/PageResult.class`，`ResultMap.class` / `FollowPageResult.class` / content 域旧 `PageResult.class` **零残留**。
> * **红线合规**：只做搬移与依赖调整；对外行为零变化（信封字段名与 `totalPages` 推导公式逐字段不变；`CommentDao` 调用点仍是同一查同一映射）；未改业务语义；无新依赖；无 DDL；未动 IoC 扫描（`ClassScanner` 扫 `com.itheima` 整根递归，新增 `common` 包 / 删空 `dao` 包均不影响 Bean 发现——新增类无 `@Component`）。
> * **覆盖边界（如实记录）**：① pytest 侧为**回归**——信封字段名未变，故 `test_follow_list`（12 条）/ `test_comment_paging` / profile 相关分页断言**一条未改写**，这正是"对外行为零变化"的端到端守卫；② `dao` 层与 `ResultMap` **无测试层**（项目无 DAO 单测），搬移正确性由"方法体逐行相等"脚本 + pytest 端到端共同保证，**JUnit/pytest 均无针对搬移本身的断言**（如实记录，未假装有）；③ 前端**零改动**（JS 只按字段名消费信封，grep 确认 `static/js/` 无类名引用）；④ 新增 `common` 包无 `@Component`，不参与 IoC。
> * **G11 记录**：L1 ×2（① `ResultMap` 反向 import 实测 **5** 条 ≠ 四要素写的 1 条；② 四要素"验收"写"包依赖**单向**"，实际拍板口径为"环**保留现状** + 显式登记"——措辞与结论不同，已按拍板结论落 CA 4.1，避免文档留一个做不到的验收）。L2/L3/L4 无——R-03 是四要素点名的**拍板前置**，已按 G7 交用户裁决，非窗口自决。
> * **文档（G4 同 commit）**：`常青/CURRENT_ARCHITECTURE.md`（头部 **3.9** + 4.1 包总览改 `dao`→`common` 与环实测注 + 4.2 `dao` 节改写为 `common` 节 + 4.3 user/content/follow/comment 四域行数与归属注）、本档三节/四节、`NEXT_CYCLE_NEEDS.md`（4.0 决策段 + 二节 R-02 / 三节 R-03 状态 + 4.3 + 头部）、`.docs/INDEX.md` 周期状态行、`UNPLANNED_ISSUES.md`（**U-07 / U-19 关闭留痕**）。**`BUSINESS_FLOW.md` 未改**——本任务零业务流程/机制变化（纯包与依赖结构调整）。
> * **commit 主题口径**：四要素"期望 commit 主题"= `refactor`（跨多域可不带 scope，合 `COMMIT_CONVENTION` §三）；实际沿用本周期令牌取 **`refactor(cache-14)`**（对齐 T10~T12 先例；`cache-` 是周期令牌而非"缓存任务"声明）。
> * **独立评审（subagent，只读）结论与处置（2026-09-20）**：**无 🔴 阻断项**，总体判定"可提交"；**🟡 2 条**（自报 2 条与实列一致）：
>   ① 🟡 **CA 6.17 残留陈旧表述**——"接口口径"行仍写"follow 域信封 `FollowPageResult`"、"包层边界"行仍写"信封落在 follow 域 / **不 import** content 域 `PageResult` / 上移公共包**登记留池**" → **已采纳**：两处改写为 `common.model.dto.PageResult` 的现实，后者补记"原 T7 规避口径与留池尾巴随公共包落地失效"。属 T14 自身制造的文档不一致（改了包却漏改引用它的章节），成本极低、必须修。
>   ② 🟡 **`dao` 映射等价性无 JUnit 覆盖**（评审建议在 `UNPLANNED_ISSUES.md` 记一行"未覆盖风险"）→ **不采纳**：本项目"JUnit 只覆盖不碰 DB/HTTP 的层"是既有**明文分层决定**（`说明书/TEST_AUTOMATION.md` §0.1），`dao` 层从来无单测，**不是 T14 引入的债**；且该事实已在本任务"覆盖边界"如实记录（含"JUnit/pytest 均无针对搬移本身的断言"），再进池属"已登记却又登记"，并把一个全局设计选择降格成待办项。等价性保证 = 6 个方法体逐行字符串比对（`temp_script/t14_verify_moved.py`）+ pytest 端到端。
>   评审另行核对通过（择要）：6 个方法体与 `git show HEAD` 逐行一致、20 处调用点前缀全清；新 `PageResult` 与两个旧类的字段 / getter / `totalPages` 公式逐字节一致；`FollowPageResult` 的"缺省不构造信封"语义确由 Controller 承担、删类未带走行为；`ProfileVO.contentPage` 仅换包名、Jackson 输出不变；`web.xml` / `pom.xml` / `tools/` 无 `com.itheima.dao` 引用；新类无 `@Component`、`ClassScanner` Bean 发现不受影响；跨域环实测 **7 个**且 `common` 零出边（与 CA 4.1 表述一致）；CA 4.3 八个行数单元格经 `wc -l` 全部吻合；`git status` 改动清单与 T14 声明完全吻合、无 G5 扩大。评审自列"无法验证项"= 编译/测试数字、war 内容、pytest 断言零改动——**三项均由本窗口实测补齐**（禁跑 mvn 的评审环境限制），非遗留空白。
> * **评审后复跑（2026-09-20，处置后同一工作树）**：`test junit` exit 0、**511 / 0 / 0 / 0**（surefire 逐类求和一致）；`test all` exit 0、`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`；**两层数字与首次完全一致**（本轮处置只改 CA 文档、未动源码）。war 内容复验：仍只有 `WEB-INF/classes/com/itheima/common/model/dto/PageResult.class`。

### T15 顺手清理：架构文档与代码一致性（N16 + N9 文档部分）

* **入口线索**：`常青/CURRENT_ARCHITECTURE.md:72` 目录树里的 `ssm_*` 实际不存在；同文件 `:135` 的 AuthFilter 精确名单只列 7 项、代码实为 10 项（缺 `/content/update`、`/content/mediaDelete`、`/content/delete`）。**（2026-09-20 T12 窗口补充；2026-09-20 T14 窗口更新）** 四.3 业务域包模块表的**行数大量滞后**：T12 已修正 content 域 service 单元格；**T14 已修正其实际改动过的单元格**（content 域 dao/model、comment 域 service/dao、follow 域 service/model、user 域 dao）；**仍待核对**：`user` 域 `UserService` 241→**250**、`like` 域 `LikeService` 205→**215**、`LikeCacheService` 394→**393**——请按 `wc -l` 实测逐格核对（含职责描述是否已随 T10/T11 失效）。目标：把"文档 vs 代码"的事实差异**一次扫完并修正**。
...
> **执行回写（2026-09-20，第七期 T15 窗口）**
>
> * **证据复核（G11 第 (0) 项）**：N16 / N9（名单部分）两条均成立并已修正——① 顶层实测 = `AGENTS.md / logs / pom.xml / src / target / temp_script / tools`，**无 `ssm_*`**；② AuthFilter 代码精确名单 **10** 项、文档 **7** 项（缺 `/content/update`、`/content/mediaDelete`、`/content/delete`）。**事实修正（L1）**：四要素"仍待核对"写的 `UserService 241→250` 已过期——T13 把该文件改到 **271**（文档 §4.3 早已是 271，实际亦 271），本窗口未改动该项。
> * **交叉核对全量清单（T15 验收要求"一次扫完"）**：① `@WebServlet` **14** 个 Servlet ↔ §7 API 表逐条对照；② AuthFilter 两个名单（5 前缀 + 10 精确）↔ §4.2 名单 ↔ `BUSINESS_FLOW` 7.1 三处同源；③ **§4.2 + §4.3 全部行数单元格**按 `wc -l` 逐格复核（含 T14 未覆盖的 cache / ioc / controller / like / admin / user-model 各表）；④ §5.2/§5.3 表结构按 **3306 实际 DDL**（G9 备份 `.docs/DBbackups/20260920_163919/db.sql`，T11-C DDL 之后）复核；⑤ §5.1/§6.1 配置项 ↔ `app.properties`（逐键一致）；⑥ §8.1 文件结构 / §8.2 路由表 ↔ `static/js`（7 个根 js + 9 个 views + 9 条 `register()` 全吻合）。**机器化证据**：一次性脚本 `temp_script/t15_verify_docs.py`（gitignored）把"文档行数单元格 / 两个鉴权名单 / Servlet URL / 控制器分发字面量"与源码对照，**输出"全部一致"**（改前报 24 条差异）。
> * **落地（CURRENT_ARCHITECTURE.md，版本 3.9 → 3.10）**：
>   * **目录树**：删 `ssm_*/`；补 `temp_script/`、`target/` 与 `.docs/报告/`、`.docs/DBbackups/`；`archive/` 子树收敛为一行（实测含 常青 / 目标与任务 / 说明书 / 报告 / DBbackups 五项，原稿只列 3 项且归属写错）。
>   * **鉴权名单**：§4.2 精确名单 7 → **10 项**；Filter 表 `AuthFilter 76 → 77`。
>   * **URL 表**：§7.2 `/search` → **`/search/keywordSearch`**——`SearchController`（`@WebServlet("/search/*")`）**没有裸 `/search` 分支**（`action == null` → "未识别功能"），前端唯一调用点 `views/search.js:158` 也用的是 `/search/keywordSearch`；原表述等于告诉读者 `GET /search?keyword=` 可用。
>   * **行数 26 处**（`wc -l` 实测，口径已在 §4.1 注明）：ioc `ClassScanner 48→47`、`@Component/@Inject/@PostConstruct 11→10`；filter `AuthFilter 76→77`；controller `BaseServlet 41→40`、`BaseServletUtil 99→101`、`RequestParser 69→**28**`；cache `CacheDomain 26→27`、`CacheStats 131→130`、`JacksonCodec 53→64`、`RedisAccess 91→90`、`SingleFlight 65→70`、`CacheResult 39→47`、`CacheAside 575→619`；user `ChangeUserNameDTO 13→15`；content `ContentController 182→181`、`FeedController 57→31`、`ProfileController 70→44`、`ContentCacheDTO 136→135`、`CommentCacheDTO 110→119`、`ContentDetailVO 26→25`、`CommentVO 22→21`；like `LikeService 205→215`、`LikeCacheService 394→393`；admin `AdminCommentController 47→46`。（明细见 `git diff`。）
>   * **表结构（§5.2/§5.3）**：comment 行 `id`→`comment_id`、`message`→**`content`**、补 **`reply_count`**；content_media 行 `media_type`→**`type`**；§5.3 补 comment 复合索引 `idx_content_parent (content_id, parent_id)`（T10-A，原稿整节缺失）。
>   * **补注**：ioc 四个注解类实际位于子包 `ioc/annotation/`；§4.1 行数快照日期 `2026-09-12` → `2026-09-20` + 口径说明。
> * **落地（BUSINESS_FLOW.md，版本 2.4 → 2.5）**：① 1.1 Filter 链图补 `ExceptionFilter`（原只画 3 个，`web.xml` 实注册 **4** 个且 ExceptionFilter 最外）；② §7.1 AuthFilter 精确名单 7 → 10 项；③ §7.2 公开接口 `/search` → `/search/keywordSearch`、补 `/comment/replies`（公开接口原稿漏列）；④ §3.5 流程图与接口定义两处 `/search?keyword=` → `/search/keywordSearch?keyword=`；⑤ §3.1 上传流程图盒内 `保存到 D:/stone` → `保存到上传根目录`（实际落盘路径由 `app.properties` 的 `upload.path` 决定，同节步骤 4 已写对；盒内不再重复硬编码本机路径）。
> * **冗余清理（用户本窗口追加要求，限**常青文档**）**：① `CA` 头部版本行由 T14 变更长叙述压为一行；② `ZSetCache` 单元格（623 行类的职责描述）与 §6.17 重复约 450 字 → 收敛为一句 + "见 6.17"指针；③ content 域 service 单元格去掉 T10-A/T10-B/T12 的历史叙述（§6.4 / §6.18 已载）；④ `BUSINESS_FLOW` 头部"最后更新"由 4 个任务的长变更叙述压为 T15 一行 + 指针。**被压掉的都是与其它章节重复的变更史**（CA 自身"维护说明"本就声明不设变更记录、历史归 git）；**未删任何"唯一承载"的事实**。周期文档（TASKS/NEEDS/INDEX）本窗口**只做事实性状态更新，不做结构改写**——按既有约定，已完成任务的表述由用户在归档前自行清理（`73bf5aa` 先例）。
> * **明确不做**：不改任何代码 / 业务语义 / 接口契约 / 配置；不做章节重写或扩写（只动事实）；**未改 `说明书/DATABASE.md`**（见 G11 L2）。
> * **验证（纯文档批，不跑测试）**：T15 验收即"`git diff` 只含事实修正" → 实测 `git status` 仅 **2 个常青文档**变更，无源码 / 测试 / 配置 / pom 改动；三项证据均可复核（`wc -l` 逐格、`grep @WebServlet` + 控制器分发分支、G9 备份 dump 的表结构）+ 脚本 `temp_script/t15_verify_docs.py` 全绿。
> * **红线合规**：只修事实错误；未改代码 / 业务语义；未扩写章节。
> * **覆盖边界（如实记录）**：① 本轮只核**事实类**章节（结构 / 名单 / URL / 行数 / 表结构 / 配置 / 前端文件与路由）——§6.x 的**机制叙述**未逐条复核（T10~T14 各窗口已随改随修）；② `说明书/*.md`（TEST_AUTOMATION / COMMIT_CONVENTION / TEST_SEED / AVAILABLE_TOOLS）未纳入；③ `BUSINESS_FLOW` 除上述 4 处外未做全量事实核对（其 §4/§6 分页口径已在 T11-B 窗口更新）；④ §8.2 路由表只核了路由↔视图映射，"需要登录"列按 AuthFilter 推断，未逐页走查前端鉴权分支。
> * **G11 记录**：L1 ×2 —— ① `§4.2 RequestParser` 文档 **69** → 实测 **28**（落差 41 行，远超常规行数滞后，疑为某次瘦身未同步；属既有滞后、非本窗口引入）；② 四要素点名的 `UserService 241→250` 早已被 T13 改写为 271（文档无错）。**L2 ×1** —— `说明书/DATABASE.md` 的手写建表语句与 3306 实际 DDL 已大面积不符（comment 缺 `reply_count` 且列名 `content`、users 缺 `role`、content 缺 `comment_enabled`/`file_exists`/`last_verify_time`、content_media 用 `type` 等）；该档 INDEX 已标注"本机 / 敏感、**不追踪**"，改不改由**用户裁决**，**本窗口未动** → 已按 G11/强制探索步骤(2) **登记池 `U-23`**（T15 窗口新登记，池内存量 3 → 4）。L3/L4 无。
> * **文档（G4 同 commit）**：`常青/CURRENT_ARCHITECTURE.md`（3.9 → **3.10**）、`常青/BUSINESS_FLOW.md`（2.4 → **2.5**）、本档一 / 三 / 四节、`.docs/目标与任务/NEXT_CYCLE_NEEDS.md`（4.1 N9·N16 去向 + 4.3 状态）、`.docs/目标与任务/UNPLANNED_ISSUES.md`（**U-23 新登记** + T15 窗口处置段）、`.docs/INDEX.md`（周期状态行 + 留池项）。**无新决策 → 未写 NEEDS 4.0 决策段**（T15 为纯文档一致性，无拍板项）。
> * **commit 主题口径**：四要素"期望 commit 主题"= `docs`，与实际一致。
> * **独立评审（subagent，只读）结论与处置（2026-09-20）**：**无 🔴 阻断项**，总体判定"可提交"；**🟡 2 条**（均为 T15 之前的既有遗留，**已顺手修**）：
>   ① 🟡 `BUSINESS_FLOW` §7.2 表格行仍写 `GET /search`（与同文件 §3.5 / §7.1 的更正不同步）→ **已改**为 `GET /search/keywordSearch`；
>   ② 🟡 `BUSINESS_FLOW` §3.1 上传流程图盒内写 `保存到 D:/stone`，与 `app.properties` 的 `upload.path=D:/data/projects/VideoPlatform/stone` 不符（同节步骤 4 已写对）→ **已改**为"保存到上传根目录"（盒内不再硬编码本机路径，避免 T18 配置参数化后二次失效）。
>   评审另行核对通过（择要）：§7 **全部 47 行 API 路径**均存在对应 `@WebServlet` + `doGet`/`doPost` 分支，**无凭空路径、无 GET/POST 标错**；"需要登录"列与 AuthFilter 两个 Set **逐项一致**；§5.2/§5.3 与 G9 dump 一致（含 `content` / `reply_count` / 三项索引）；**"3 张遗留表零引用"经独立 grep 复核成立**（`video` 仅命中 `UploadType.VIDEO` 枚举与 `/upload/video/` 字面量，非表名引用）；§4.2/§4.3 的职责描述未随 T10~T14 过期；4 处压缩掉的内容在别处仍有承载（**无误删唯一承载事实**）；`git status` 只有 6 个文档、无越界。
> * **评审后复核（2026-09-20，处置后同一工作树）**：`temp_script/t15_verify_docs.py` 复跑仍**"全部一致"**；`grep` 全库确认 `.docs/`（除 `archive/`、`temp/`）内**再无** `GET /search?` / 裸 `/search` 表行 / `D:/stone` 残留。**纯文档批，未跑测试**（T15 验收口径）。
