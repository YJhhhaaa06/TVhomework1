...
> **执行回写（2026-09-20，第七期 T11-B 窗口）**
>
> * **落地（代码）**：① `CommentController` 新增常量 `COMMENT_PAGE_SIZE_DEFAULT = 200`（150 → 160 行），`/show` 分页分支与 `/replies` 两处改走三参重载 `parsePageSize(req, 500, 200)`——**上限 500 不变、缺省信封 10 → 200**；`hasPagingParams` 分支（缺省全量数组）**保留**（评论域信封形状不动）。
> * ② `static/js/chunkedList.js`（65 → 124 行）增：`keyOf`（缺省 `userId` → `commentId` → `id`，带类型前缀）+ 内部 `seen` 集合（**批内与跨 chunk 双向过滤**）、`nextBatch` 批不满时继续消费本地余量/续拉下一 chunk（按**未提交的消费位置**判定，`batchSize > chunkSize` 亦成立；单次最多 10 个 chunk，防"整页重复"死循环）、`reset()` 清 `seen`、**信封大小自适应**（首次响应回显 `pageSize` 覆盖 `chunkSize`，`chunkSize` 降级为初始/兜底值）；请求抛错时 `cursor`/`seen` **不提交**（不吞条目）。
> * ③ `views/detail.js`（575 → 581）：评论主楼 `fetchChunk` **只传 `page`** + `keyOf: c => c.commentId`；展开回复 `/comment/replies` 同样只传 `page`（原硬传 50 → 后端 200）。
> * ④ 三处内容列表迁移（`chunk = 50` 顶公共 cap、`keyOf: it => it.id`）：`views/follow.js`（`/feed`，小批 10，`loadFeed(page)` 拆为 `loadFirst` + `loadMoreFeed`，删 `setLoadMore`，132 → 144）、`views/search.js`（小批 12，类型筛选仍在"已展示集合"上做，`total` 由 `fetchChunk` 旁路捕获，219 → 226）、`views/publish.js`（我的投稿，小批 12，563 → 622）、`views/user.js`（创作网格 `/profile`，小批 10，352 → 386）。
> * **前端重渲染口径（有意变更）**：`publish.js` 的"删除模式切换 / 删除卡片 / 编辑保存"由"重拉当前页"改为**本地条目集重渲染**（`state.myItems`；编辑走 `search/IdSearch` 定向刷新单条）——同时消除原实现"在第 N 页切换删除模式/删除后把第 N 页卡片再追加一遍"的既有缺陷；四处"加载更多"统一为按钮内联 loading + 失败复原并 toast（与 T11-A sheet 同构）。`user.js` 的头部信息（用户名/关注数/粉丝数/创作总数）改由 `fetchChunk` 旁路捕获的 `state.profile` 提供，仅在首批后渲染一次。
> * **否决/未采纳**：① 不改 `/feed`·`/search`·`/profile` 的后端上限与缺省（红线；后端补齐归 **T19**）；② 不删评论域 `hasPagingParams`（"缺省全量数组"本轮不动，与 follow 域 T11-A 的反转不同——红线：不改信封形状）；③ 不改 `chunkedList` 缺省 `keyOf` 顺序（按拍板 `userId`→`commentId`→`id`），改用"调用点显式传 `keyOf`"兜住评论 VO 陷阱（见 G11 记录②）。
> * **验证（落盘证据）**：`python tools/tv.py test junit` **exit 0**、`latest.json` = **482 / 0 / 0 / 0**；`python tools/tv.py test all` **exit 0**（`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`）。**用例数不变**（JUnit 482 → 482，无 Java 测试改动；pytest 145 → 145，仅断言改写与补断言，无新增/删除用例）。另做**一次性 Node 行为核验**（`node --check` 6 个改动 JS 文件 + `createChunkedList` 行为：130 条 / 3 请求、唯一性 130、整页重复不死循环、回显 `pageSize` 自适应、缺省 `keyOf` 在"同作者评论"上确实折叠（3 → 2，印证显式 `keyOf` 必要性）、抛错后重试不丢条目）——过程命令属窗口内临时核验，**未落库**（项目无 JS 测试层）。
> * **pytest 改写清单（逐条）**：`test_comment_paging.py` ① 头部契约注记"pageSize 默认 10"→"信封 200 / 上限 500（T11-B）"；② `:181` `by_page["pageSize"] == 10` → `== 200`（补注释）；③ `test_replies_envelope_and_out_of_range` 补"只传 `page` → `pageSize == 200`"断言。**后端「页间主楼不重不漏」（`test_pages_cover_all_roots_without_overlap`）等契约断言一条未改**。
> * **红线合规**：不改 `/feed`·`/search`·`/profile` 后端上限与缺省（三个 Controller 零改动）；不改 `/comment/*` URL 与信封形状（`/show` 缺省仍是全量数组）；不动评论缓存结构（T10-A/T10-B 定稿）；无新前端依赖、保持原生 ESM；去重只兜 offset 漂移、不替代后端正确性。
> * **G11 记录**：① **L1**：`SearchController.search` 的 `pageSize` **不走** `BaseServletUtil`（缺省 12、**无上限**，直接进 `SearchDTO`）——与 `feed`/`profile` 的公共 cap 50 不同源，属既有事实；本任务按拍板统一取 `chunk = 50`（不放大），"三域上限归一"归 **T19** 一并参数化。② **L1**：`chunkedList` 缺省 `keyOf`（拍板 `userId` 优先）与**评论 VO 同时含 `userId`（作者）**冲突 → 缺省会把同一作者的评论折叠（Node 实测 3 → 2）；已在 helper 头部注释、常青 6.19、调用点显式 `keyOf` 三处兜住，**缺省顺序按拍板不动**。③ **L1（顺带事实修正）**：`BUSINESS_FLOW` 3.1 / 4.2.2 中"评论缓存仍是整树 JSON / 展示层切片"的表述**自 T10-A 起已失效**，本任务触及相关段落时一并改为现状（两键组 + 主楼窗口 + 前 K + 展开接口）并指向 6.18；架构文档 comment 域模块表的 `CommentController` 行数原记 124（改动前实测 150）已改为 160。
> * **覆盖边界（如实记录）**：项目**无 JS 测试层** → `chunkedList` 的去重/自适应/防死循环、四处视图迁移与"本地余量内 0 请求"**无自动化断言**，仅代码审查 + 上述一次性 Node 核验；本轮未引入 JS 测试依赖（前端测试层缺口的归属待后续评估）。
> * **文档（G4 同 commit）**：`常青/CURRENT_ARCHITECTURE.md`（头部 3.5 + comment 域模块表行数/职责 + 6.17 消费方措辞 + 6.18 接口口径与前端段 + **新增 6.19「前端列表分块与去重」** + 8.1 前端目录树 6 行 + API 表 `/comment/show`·`/comment/replies`）、`常青/BUSINESS_FLOW.md`（头部 2.2 + 3.1 评论分页口径与失效表述修正 + 4.2.2 分页段 + 前端交互注 + 接口清单行）、本档三节/四节、`NEXT_CYCLE_NEEDS.md` 4.0/4.2/4.3、`INDEX.md` 周期状态行。
> * **待用户裁决**：无（本轮无 L3/L4 升级项）。
> * **独立评审（subagent，只读）结论与处置（2026-09-20）**：**无 🔴 阻断项**，总体判定"可提交"（评审自报 3 条 🟡、实际逐条列出 2 条，按 2 条处置）：
>   ① 🟡 `ensureChunk` 用**已提交**的 `cursor` 判定"本地是否还有余量"，而 `nextBatch` 用**未提交**的 `pos` 消费 → `batchSize > chunkSize` 时"批内续拉下一 chunk"静默失效（当前 5 处调用点均 `batchSize ≤ chunkSize`，未触发，属潜伏缺陷）→ **已采纳**：`ensureChunk(pos)` 改按未提交位置判定；Node 复验 `batchSize=120 / chunk=50` 单批取满 **120（3 请求）**、第二批 **80（＋1 请求）**，常规路径（130 条 / 3 请求）不变。
>   ② 🟡 极端"整页皆重复"触发 guard 上限时返回空/短批，调用方 `if (!batch.length)` 可能提前收尾 → **维持**：首屏（page 1）不可能整页重复（此时 `seen` 为空）；中途出现时 `hasMore()` 仍为真、"加载更多"按钮仍在，再点即继续，不会永久丢数据；彻底消除需给 helper 扩"未取满"信号（语义扩张），收益不抵风险，如实记录。
>   评审同时核对通过：helper `pending`/`seen` 提交时序、`keyOf` 在响应 `pageSize` 为 0/负/缺失时的兜底、四处视图空态/错误态/终止条件与悬空引用（`state.page`/`loadFeed`/`setLoadMore`/`loadProfile`/`renderProfile`/`myPage` 等零残留）、`CommentController` 两处三参重载与 `min(defaultSize,max)` 语义、`/search/keywordSearch` **不走**公共解析（缺省 12、无上限；前端显式传 50 故实际回 50，不会造成 `loadedAll` 误判）、测试非空转（"页间不重不漏"未改）、文档行数与实测一致、全库无"整树/全树"旧表述残留；红线逐条未违反（本次仅 `CommentController` 一个 Java 文件被改）。
> * **评审后复跑复核（2026-09-20，处置后同一工作树）**：`test junit` exit 0、`{tests: 482, failures: 0, errors: 0, skipped: 0}`；`test all` exit 0、`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`。**与首次数字一致**（本轮处置仅 JS helper 一处判定改动，不动两层测试的用例集）。
...
> **执行回写（2026-09-20，第七期 T11-C 窗口）**
>
> * **commit 结构（按拍板 Q3 拆两 commit）**：**C-1** = DDL + 窗口 DAO（纯新增、行为零变化）；**C-2** = 前缀装载 + 判定回落 + 写路径 `partial`。**文件级切分干净**（便于用户分两次提交）：C-1 = `follow/dao/FollowDao.java`（DDL 无文件）；C-2 = `cache/CacheKeys.java`、`cache/ZSetCache.java`、`follow/service/FollowCache.java` + 两个测试类 + 文档。
> * **落地（C-1）**：`FollowDao` **107 → 155 行**，新增两个**窗口查询**——`getFollowedUserIdsInWindow(conn, userId, offset, count)`（`SELECT followed_user_id … WHERE user_id = ? ORDER BY followed_user_id LIMIT ? OFFSET ?`）与 `getFollowerUserIdsInWindow`（同构，键为 `user_id`/`followed_user_id`）；均带 `count <= 0 || offset < 0 → 空列表` 的防御守卫。**本 commit 不接线，行为零变化**（编译校验 `mvn -o -q compile` exit 0，经 `temp_script/t11c_compile.py` 一次性脚本执行）。
> * **DDL（G9 闭环，C-1）**：语句 = `ALTER TABLE follow ADD KEY idx_followed_user_user (followed_user_id, user_id);`，**执行前已将原文交用户过目**。闭环证据：① 改前备份 `.docs/DBbackups/20260920_163857/db.sql`（60448 bytes，含 manifest+sha256）；② ALTER 3306（生产 `tvdatabase`）；③ 改后备份 `.docs/DBbackups/20260920_163919/db.sql`（60511 bytes）；④ 改后备份归档为测试库重建源 `.docs/archive/DBbackups/20260920_163919/`（dump 内含 `idx_followed_user_user`）；⑤ `tv.py init-test-db` 重建 3307（content 62 行 + 基线种子校验通过：管理员 role=1 / 基线评论 3 条 / 固定券库存 999999）；⑥ 两库 `SHOW INDEX FROM follow` 均含新索引；⑦ **EXPLAIN 实测**：关注方向走 `uk_user_follow`、粉丝方向走 `idx_followed_user_user`，两方向 `Extra` 均为 `Using index`（覆盖索引）且**无 `Using filesort`**。
> * **落地（C-2，代码）**：
>   * `cache/CacheKeys` **172 → 193 行**：新增 `PARTIAL_MARKER_VALUE` 常量与 `partial(dataKey)` 工厂（`partial:{dataKey}`，与 `empty:` 同构）；`domainOf` 增 `partial:` 解包（统计归域不落 OTHER）。
>   * `cache/ZSetCache` **403 → 623 行**：四读方法全部接部分态语义——① `getWindow(zsetKey, offset, count, WindowLoader, Supplier<Long> totalLoader, ttl)`：**完整态**仍是"探针 + 一趟 pipeline `ZRANGE`+`ZCARD`"（与 T7 逐字节一致）；**miss** 改为前缀装载 `[0, offset+count)`（取满 → 置 `partial:`，不足 → 到底即完整）；**部分态** `readPartialWindow`：页落在前缀内零 DB、越界只补 `[W, offset+count)`（单飞内重读 ZCARD 取水位，防拿陈旧 W 重复追加）；**降级** 改为 DB 窗口直查（单飞去重 + 窗口指纹 key `zsetKey@offset+count`，**不装载不写回**），total 走注入的计数口径。② `isMember` 增 `Supplier<Boolean> dbAnswer` 参数：`partial:` 存在且 ZSCORE 未命中 → 回落 DB 单行（命中照旧直接 true）。③ `batchIsMember`：部分态下未命中成员并入既有 `dbAnswer` 批次，且**跳过全量回填**（不把装载量重新放大到 O(总量)）。④ `getMembers`：遇 `partial:` 走 `completePartial`（全量 loader + ZADD **合并** + 清标记），并**直接返回 DB 装载结果本身、不回读 Redis**——写回是 best-effort（失败只记 WRITE_FAIL），回读只能拿到前缀，那正是"静默漏关注者"（评审 🔴 修复点，见下）；**绝不返回前缀**。⑤ 探针/批量 pipeline 增 `exists(partial:)` 且 `expire(partial:, ttl)` —— **`partial:` 标记与数据 key 同步续期**（生命周期错位会让前缀被误判为完整集，属数据正确性问题）。⑥ 新增私有 `writeWindowLoad`（ZADD 合并 + EXPIRE + 置/清标记，空且到底 → `markEmpty` + 清残留标记）、`clearPartial`、`cardinality`、`readPage`；删除已无用的私有 `slice`。
>   * `follow/service/FollowCache` **446 → 539 行**：`getFollowingWindow`/`getFollowerWindow` 换传 **`WindowLoader`（DAO 窗口 SQL）** + **`totalLoader`（本域计数口径 `getFollowCount`/`getFollowerCount`）**；`isFollowing` 增 `isFollowingInDb` 单行回落；新增 `loadFollowingWindow`/`loadFollowerWindow`；`probePair` 扩为**六探针**（两条 data key 的 exists/empty/partial）并以 `Probe.memberWritable()` 表达"两侧已加载且均非部分态"，`cacheFollow`/`cacheUnfollow` 的增量写条件改用它（**任一侧 `partial:` → 三件套双 DEL**）；`invalidateKeysQuietly` 扩为 DEL 数据 key + 空标记 + `partial:` 标记，并新增 `invalidatePairQuietly`（写路径 Redis 异常分支的兜底失效改走**同一三件套口径**——原 `CacheAside.invalidate` 只删数据 key + 空标记、会残留 `partial:` 标记；评审 🟡 采纳项）。**删除 `getFollowerIds`（池 U-21 处置：确认仍有零主代码调用方）与其单测**，连带删除私有 `loadFollowerIds`（粉丝方向全量 loader 随窗口装载作废——粉丝侧已无任何全量语义需求）。
> * **与拍板方案的两处实现层偏离（不改变拍板结论，已在此说明）**：① 拍板写"多传 `partialKey`"，实际**改为组件内从 `zsetKey` 派生**（`CacheKeys.partial(zsetKey)`）——与既有 `empty:` 完全同构，省掉 6 个调用点的漏传风险，且 key 命名仍唯一收敛在 `CacheKeys`；② 拍板写"total 走计数 key（CacheAside；再失败 → DAO 单列）"，实际以 **`Supplier<Long>` 注入**（由 follow 域提供 `getFollowCount`/`getFollowerCount`，其内部已含"miss 回落 `users` 表计数列"的兜底）——**缓存基建不反向感知 follow 域**，口径与拍板一致。
> * **否决/未采纳**：① `getMembers` 补齐**不用**"DEL 后 ZADD"（会丢并发写），保持**合并**语义（残余竞态见 G11 ①）；② 不回退成"任意窗口装载"或"分页直读 DB"（α/β 已否）；③ 不改对外信封与排序口径（`FollowPageResult` 字段、id 升序一概不变）；④ 不在 pytest 引入 Redis 客户端（拍板 R6）；⑤ `/feed`·`/search`·`/profile` 的后端上限仍归 **T19**。
> * **验证（落盘证据，评审处置后复跑）**：`python tools/tv.py test junit` **exit 0** → `.stage8-target/test-reports/latest.json` = **500 / 0 / 0 / 0**（26 个 surefire 报告逐类汇总亦 500/0/0/0）；`python tools/tv.py test all` **exit 0**（`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`）。⚠️ 中途如实记录：junit 首跑有 1 处失败（`getFollowingWindowPartialWithinPrefixDoesNotLoad` 的 `verify(tt, never())` 与"部分态 total 走计数 key"冲突——计数 key 未命中时本就会回落 DB 取计数列），改为"计数 key 命中"前提后复跑全绿；此为**断言前提写错**，非实现缺陷。
> * **用例数变化**：JUnit **482 → 500（+18）**——`ZSetCacheTest` **26 → 35**（+9：部分态判定回落、部分态命中免 DB、部分态补齐、**写回失败仍返回 DB 全量（评审 🔴 回归）**、装载即到底转完整态、部分态页内零 DB、越界只补差量、补齐到底清标记、批量的部分态回落且不回填）；`FollowCacheTest` **43 → 52（+9）**（+10 新增：部分态单行回落、部分态命中免 DB、批量部分态回落、全量读补齐、部分态页内零 DB、越界只补差量、写路径异常分支三件套双 DEL ×2、Redis 全挂不抛出 ×2；−1 删除 `getFollowerIdsReturnsFollowersFromCacheOrDb` 随方法删除）。pytest **145 → 145（不变）**：本任务未改任何 pytest 断言（对外契约零变化，故 `test_follow_list.py` 逐条原样通过——它同时也是窗口 SQL 的端到端验证层）。
> * **红线合规**：① **缓存失败不得导致业务失败**——窗口装载/补齐写回失败只记 WRITE_FAIL 不抛出；降级不写回；② **loader 抛 `ServerException`（500 语义）→ 不写任何 key、不写空标记**（`writeWindowLoad` 只在 loader 正常返回后调用）；③ **不改对外信封形状与排序口径**（`FollowPageResult` 字段集、id 升序、越界空页保留 total 均不变）；④ **不引入新依赖**（只用 Jedis 既有命令）；⑤ **不碰 `buildUserList` 事务边界**（属 T12）；⑥ `getFollowingIds` 的 feed 全量关注 ids 读路径**保留**（R-01 明确保留，只在其上增加"遇部分态补齐"）；⑦ 不擅改 `@WebServlet` URL / web.xml / IoC 扫描。
> * **G11 记录（L1，仅记录不打断）**：
>   ① **补齐合并语义的残余竞态**：`getMembers` 的补齐用 ZADD **合并**（不 DEL，避免丢并发写）。若某次取关恰好落在"读 DB 全量"与"ZADD"之间、且该次取关因 `partial:` 已双 DEL 生效，则合并结果会保留那个已被取关的成员，直到该 pair 下一次写操作失效自愈。窗口补齐路径（只追加 `[W, offset+count)`）无此问题。属"合并 vs DEL 各有洞"的取舍，按拍板口径实现并如实登记。
>   ② **部分态判定回落 DB 无独立统计事件**：`isMember`/`batchIsFollowing` 的"部分态未命中 → DB"不计 MISS/DEGRADE（未扩 `CacheStats.Event` 枚举）。可观测性缺口，随观测/日志方向评估。
>   ③ **冷 key 分页路径往返数上升**：完整态命中仍与 T7 相同（探针 + 窗口两趟）；冷 key / 部分态多出 ZCARD 与域级计数读——代价换"装载量 O(列表总量) → O(页)"，与 U-18 目标同向。
>   ④ 架构文档 5.2 的 `follow` 表关键字段仍未列 `create_time`（既有滞后，属 **T15** 范围，本次未顺手改）。
> * **覆盖边界（如实记录）**：① **DAO 窗口 SQL 无单测**（项目无 DAO 测试层）→ 由 pytest 端到端分页用例（真实库、真实索引）+ EXPLAIN 佐证；JUnit 覆盖的是"装载量 / 标记置清 / 回落 / 补齐 / 双 DEL / 降级不装载"的组件与时序语义（loader 打桩）。② pytest 侧无 Redis 客户端（拍板 R6）→ 水位/标记类断言不进 pytest。③ "部分态 + 并发写"的窗口竞态无自动化断言（需真实并发 + 真实 Redis）→ 仅代码审查。
> * **文档（G4 同 commit）**：`常青/CURRENT_ARCHITECTURE.md`（头部 3.6；cache 包 `CacheKeys` 193 / `ZSetCache` 623；follow 域 `FollowCache` 539 / `FollowDao` 155；5.3 索引；6.2 key 表新增 `partial:{dataKey}` 行 + `user:following` 行；6.3 `domainOf`；6.7 降级落点；**6.17 标题与三段重写**）、`常青/BUSINESS_FLOW.md`（头部 2.3；3.1 关注读路径注；4.3.1 步骤 7；4.3.3 步骤 1 与新增"装载与 total 口径"段）、本档三节/四节、`NEXT_CYCLE_NEEDS.md` 4.0/4.1/4.2/4.3、`INDEX.md` 周期状态行、`UNPLANNED_ISSUES.md`（U-18 处置完成、U-21 已删除方法并移出池）。
> * **待用户裁决**：无（本任务无 L3/L4 升级项；拍板 Q1~Q9 全部沿用）。
> * **独立评审（subagent，只读）结论与处置（2026-09-20）**：总体判定"**需修改后提交**"——**1 条 🔴 + 3 条 🟡**，已全部处置：
>   ① 🔴 **R3 补齐失败会返回前缀**（原实现"补齐后回读 Redis 作答"，而写回是 best-effort：`writeWindowLoad` 写失败只记 WRITE_FAIL 不抛出，此时 Redis 里仍是被 `partial` 标记的那些成员 → `ZRANGE 0 -1` 只能拿到前缀 → feed 静默漏关注者，违反红线①）→ **已采纳**：`completePartial` 改为**返回 DB 装载结果本身**（权威答案；单飞下 leader 与 follower 都能拿到同一结果），不再回读缓存；`getMembers` 的 Javadoc 同步说明"部分态补齐路径返回 loader 原始顺序"（`getFollowingIds` 已用 `sortIds` 归一，对外顺序不变）；**新增 JUnit `getMembersPartialReturnsDbListEvenWhenWriteBackFails`**（ZADD 抛错 + 缓存只有前缀 1 条，断言返回 DB 全量 4 条）。评审命中设计自标的最危险点，处置后补齐回归用例。
>   ② 🟡 **写路径 Redis 异常分支残留 `partial:` 标记**（原走 `CacheAside.invalidate`，其内部 `deleteQuietly` 只 DEL 数据 key + 空标记）→ **已采纳**：新增私有 `invalidatePairQuietly`，复用 `invalidateKeysQuietly` 的**三件套**口径，并加"兜底 DEL 也失败"的静默日志与 WRITE_FAIL 打点；两条既有异常用例改写为"首次 `executeVoid` 抛错 → 兜底三件套双 DEL"与"Redis 全挂不抛出"两条。**影响面**：残留 `partial` 会被下一次读装载覆盖/清除，**不致持久错误**，但会让下一次装载被误判为"仍是前缀"（total 走计数口径 + 多余补齐），故一并修。
>   ③ 🟡 **`Integer.MAX_VALUE` 夹取后可请求巨量 `LIMIT`**（`want = (int) Math.min(offset + count, Integer.MAX_VALUE)`）→ **维持（不采纳）**：该 `min` 的**存在意义是防 int 溢出**——不夹取时 `(int)(offset + count)` 可能变负 → DAO 守卫返回空列表 → 被误判"已到底"→ 把集合标成"完整且空"（**数据错误**）；夹取后最坏情形是"把可装载的行都装进来"，与 T11-C 之前的"miss 全量装载"**等价、不构成回退**；且 `page` 无上限是 `BaseServletUtil.parsePage` 各域**既有**属性（不属本任务范围）。取舍已在此说明。
>   ④ 🟡 **部分态判定未命中逐成员回落 DB 的成本** → **维持**：即拍板形态（"前缀里查不到不可信"），批量路径复用既有批量 `IN` 查询把 DB 趟数收敛为 1；大批量同时未命中属设计预期，不再收缩。
>   评审同时**核对通过**：完整态与 T7 逐字节等价（`RANGE`+`ZCARD` 单 pipeline；探针新增 `exists(partial)`/`expire(partial)` 在完整态均为 no-op；空标记不续期与精确 TTL 不变）；部分态窗口边界（`need<=0` 保持前缀、`want<=0`、补齐前重读水位防陈旧 W、`reachedEnd` 判定）；**SingleFlight 无同 key 自嵌套**（窗口指纹 key 与数据 key 不同；`completePartial` 用数据 key 但不在任一 `singleFlight.get` 的 lambda 内被同 key 再调 → `putIfAbsent + FutureTask.get()` 的死锁条件缺席）；loader 抛异常时不写任何 key（红线②）；`writeWindowLoad` 空集分支（`reachedEnd=true` 即 DB 真到底，标完整正确）；U-21 清理（`getFollowerIds` 全仓无代码引用、无悬空 `loadFollowerIds`）；`FollowDao` 窗口 SQL 升序口径与 score=id 同源；文档行数（`CacheKeys` 193 / `ZSetCache` 623 / `FollowCache` 539 / `FollowDao` 155）与 `wc -l` 一致（评审核对时点在处置前，为 612/522；处置后行数已同步更新）；无"U-18 待治/全量装载"过期表述；测试非空转（`failLoader`/`failTotal` 不被调用即验证、`RecordingLoader` 断言的是装载水位）。
>   评审的**无法验证项（如实记录）**：pytest 145 条未逐条读（采信实测数字）；EXPLAIN / DDL 闭环未重跑 DB（读留存文件）；6.17 与 5.3 仅核关键句；多实例下 `partial` 标记一致性未评估（单 Tomcat 假设）。
> * **评审后复跑复核（2026-09-20，处置后同一工作树）**：`test junit` exit 0、`{tests: 500, failures: 0, errors: 0, skipped: 0}`（26 个 surefire 报告逐类汇总亦 500/0/0/0；`ZSetCacheTest` 35、`FollowCacheTest` 52）；`test all` exit 0、`build_ok=True`、pytest **145 passed / 0 failed**、`port_18080_open_after=false`。**用例数较处置前 +3**（🔴 回归 1 条 + 写路径异常分支拆 2 条），pytest 不变。
