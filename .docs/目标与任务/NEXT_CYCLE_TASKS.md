...
| T4 | 写路径的失败与竞态治理 | N3、N4、N7 | —（独立，可并行窗口） | 空标记写入守卫生效（并发不回填假空）；索引写失败可自愈或至少可检测；条件写竞态有明确结论（修掉 or 记录为已接受）；**含 3 项，允许拆多 commit（G1 例外标注）** | `fix(cache-04a/04b/04c)`（G1 校准 2026-09-14：T4 内部拆 3 commit，a=①N3 守卫、b=②N4 索引自愈、c=③N7 条件写竞态，任务编号引用仍用 `T4`） | 执行中（04a/04b 完成 2026-09-14） |
