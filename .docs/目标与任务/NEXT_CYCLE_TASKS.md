...
> 状态：**第七期（2026-09-19 建清单）**——周期主线方向**尚未拍板**，本清单先行落位 5 个任务（编号自 **T10** 起，承接第六期 T1~T9）：**T10 评论分页 / T11 关注·粉丝列表分页**（两项为**预告条目**：只写意图与前置，**范围与四要素由执行窗口调查后拍板并回写**）+ **T12~T18** 清理类（事务边界代码债 / 注册登录兜底 / 包层结构清扫 / 文档与代码一致性 / Controller 基建收敛 / IoC 注入可观测 / 配置卫生）。
...
> 明确不做（本轮已定）：offset→游标（属 **feed 流方向**——该方向路线已预告为"读扩散查询 → 每人维护收件箱（写扩散）"）；日志体系改造相关（`U-15` 已移出池，随该方向立项盘点）；`R-01` 索引全量读（明确保留）；`U-11` 停机兜底推荐（留池，待产品决策）；NEEDS 4.1（N9~N16）中**已点单的已并入清单**：N9 文档部分→**T15**、N11→**T16**、N12 前半→**T17**、N13→**T18**、N14→**T14**、N15→**T11**（顺带）、N16→**T15**；**未排的**：N9 的"声明式鉴权"、**N10**（admin 角色每请求查库）、N12 的"声明式事务"——属**新功能方向的能力建设**，待该方向立项时评估。
...
| G9 | DDL 备份 | **本周期 DDL 未定**——T10 的评论结构改造**可能涉及 `comment` 表索引调整**（待窗口评估）。若出现任何表结构改动 → 执行前先备份库结构与建表语句到 `.docs/DBbackups/`，并向用户报备 |
...
> **DDL 标注（G9 要求）**：**本周期 DDL 未定**——T10 的评论结构改造可能涉及 `comment` 表索引调整（窗口内评估）；出现表结构改动前先备份 `.docs/DBbackups/` 并报备。

| 编号 | 标题 | 对应候选 | 依赖 | 验收关键（动态） | 期望 commit 主题 | 状态 |
| -- | -- | ---- | -- | -------- | ------------ | -- |
| T10 | 评论分页（后端大分页 + 前端小分页） | NEEDS 4.2 ① | 用户拍板红线（T8"不动缓存装载结构"） | **预告**：范围与四要素由窗口调查后拍板并回写 | 待定（窗口内定） | **预告** |
| T11 | 关注/粉丝列表分页（后端大分页 + 前端小分页） | NEEDS 4.2 ② | 无硬前置 | **预告**：同上 | 待定（窗口内定） | **预告** |
...
> **顺序理由**：**T12~T18 互相独立、可任意穿插**——**T15（纯文档）可最先做**；T12 / T16 / T17 / T18 属小改动但都要跑测试回归；T14 需先拍板 **R-03**。**T10/T11 是用户点名主菜，但都是预告条目**：进窗口前必须先做范围拍板（T10 还须先放开 T8 红线、并评估是否动 `comment` 表索引），拍板结论回写本清单四要素后方可动手。
...
### T10 评论分页（后端大分页 + 前端小分页）

> **预告条目**：本节只登记**意图与前置**——**范围与四要素（入口线索 / 红线边界 / 强制探索步骤 / 验收）由执行窗口实地调查后拍板并回写本节**（G5/G7 允许回写）。在补完四要素前**不进入执行**；补写时若发现目标需改语义/扩范围 → 按 G5 停下登记。

* **意图**：让评论分页真正"大"起来——命中路径的单次成本与评论总量**弱相关**、请求数下降。
* **已知方向（用户 2026-09-19 澄清）**：**把评论主楼与楼中楼缓存分开**（现为 `content:comments:{id}` 整树单键），**甚至可能值得动数据库索引**（窗口内评估 `comment` 表索引与查询形态）；前端配合"大 chunk 拉取 + 本地小批展示"。
* **已知事实**：T8 的 D1=A 只做了展示层切片，命中路径仍每次反序列化整树（留池项 **U-20**，已随本任务从池内移出）；切片接缝已收敛为 `ContentService.sliceRoots` **唯一方法**（换取数方式不必动调用方）；分页信封与缺省兼容口径见 `常青/CURRENT_ARCHITECTURE.md` 6.18。
* **前置（开工前必须由用户拍板）**：① **重新定义 T8 红线"不动缓存装载结构"**（本任务正是要动它）；② 若改动 `comment` 表索引 → 按 G9 先备份库结构到 `.docs/DBbackups/` 并报备；③ 窗口内先做**方案拍板**（缓存侧窗口读 vs DB 窗口读等形态比较），结论回写 NEEDS 4.0 与本清单。
