...
> 状态：**杂务/准备周期（分支 `prep/next-phase`）**——不做正式功能方向，处理代码卫生与铺垫清理。方向（R-03）已于 2026-09-18 拍板拆任务：T1~T6 第一批（基座/卫生，优先执行）+ T7~T9 第二批（用户点名主菜）。**进度：T1~T8 已完成（T7 关注/粉丝列表分页、T8 评论列表分页均已落地，决策见 NEEDS 4.0 决策段）；下一窗口 T9（测试目录本地化）。**
...
| T7 | 关注/粉丝列表分页 | N11a（U-12） | T5 | `/follow/following\|followers` 支持分页参数、默认兼容；缓存/DAO 分页载体定稿；前端 user.js 分页 | `feat(prep-07)` | **已完成**（A1+B2；JUnit 472 / pytest 135 全绿） |
| T8 | 评论列表分页 | N11b（U-13） | T5 | `getCommentsForContent` 主楼分页 + 楼中楼整树；默认兼容；前端 detail.js 加载更多 | `feat(prep-08)` | **已完成**（D1=A；JUnit 482 / pytest 142 全绿） |
...
* **执行回写（2026-09-19，feat(prep-07) 已落地）**：**强制探索结论**：N11a 证据复核成立（`FollowController` 无分页参数全量返回 / `FollowService` 全量 ids→全量装载 / `SetCache.getMembers` `SMEMBERS` 全量）；**入口线索事实修正（L1）**：原文指向前端 `static/js/views/follow.js`，该文件实为 `#/follow` **关注流（/feed 分页）视图**，真正渲染关注/粉丝列表的是 `static/js/views/user.js` 的 `openUserList()`（sheet 抽屉）→ 落点改 **user.js**；**缺省兼容口径（L2）**：旧 pytest 断言空列表时 `data == []`，故"缺省全量"**不能**以"默认 page=1&pageSize=50"实现（上限 50 会截断全量）→ Controller 显式判"是否传了分页参数"。**选型（用户拍板 A1+B2，决策段见 NEEDS 4.0）**：载体 **A1 = 成员 key Set→ZSet 同构替换 + 基建新增 `ZSetCache` 按序窗口读**（否决 A2 缓存全量内存切片：契约对但每请求仍 `SMEMBERS` 全量+装箱，成本与总量正相关、无游标锚点，抵消"后端大分页+前端预加载"意图；否决 A3 DB `LIMIT/OFFSET`·keyset 直查：能力同级且更省，但列表读路径离开缓存优先惯例、hit/miss 两套行为需分别回归）；结构 **B2 = 传 `page`/`pageSize` 任一返回信封、两者都不传仍返回全量数组**。**落地（代码/测试 8 改 + 3 新增；文档 6 改）**：① 新增 `cache/ZSetCache`（403 行）——`isMember`/`getMembers`/`batchIsMember`/`writeZSet`/`loadViaSingleFlight` + **`getWindow(key,offset,count)`**（一趟 pipeline `ZRANGE[start,stop]`+`ZCARD` → `Window{ids,total}`；hit/miss-回填后重读/降级-全量升序切片三路径同序）；② 新增 `follow/model/dto/FollowPageResult`（79 行，同形信封，**不 import content 的 `PageResult`**——content 已 import `follow.FollowCache`，反向引用会形成 follow↔content 新包层环；"PageResult 上移公共包"转留池 **U-19**）；③ `FollowCache`（446 行）读路径切 ZSetCache 并新增 `getFollowingWindow`/`getFollowerWindow`，写路径 `SADD/SREM→ZADD/ZREM`（score=成员 id；MULTI 条件双写/双 key 探针/空标记/失败双 DEL 骨架全保留）；④ `FollowService`（190 行）新增分页读（窗口取该页 ids+total → **仅该页**做 `findUsersByIds` 与 `batchIsFollowing`；信封在事务外组装）+ 抽 `loadUserList` 供缺省路径共用（**事务边界与 U-14②号点未动**）；⑤ `FollowController`（114 行）两 case 分支 + `hasPagingParams`；⑥ 前端 `user.js`（341 行）sheet 分页 + 「加载更多」（复用 `.load-more-btn`，切 following/followers 重置页码）；⑦ 新增 `ZSetCacheTest`（26 例）+ `FollowCacheTest` Set 桩整体改 ZSet 桩并补 6 例窗口读；⑧ `FollowServiceTest` +6 例分页；pytest `test_follow_list.py` +5 例（信封字段/页间不重不漏与全量前缀一致/参数归一/越界页保留 total/半参数）。**验证（沙箱外，只信落盘证据）**：`python tools\tv.py test junit` → `latest.json` **472 / 0 / 0 / 0**（434 → +38：ZSetCacheTest 26 + FollowCacheTest 窗口 6 + FollowServiceTest 分页 6）、exit=0、`build_ok=true`；`python tools\tv.py test all` → pytest **135 passed**（130 → +5）、`build_ok=true`、`port_18080_open_after=false`、exit=0。**过程返工 2 次**：① testCompile 失败（Mockito 校验 `getAllFollowedUserIds` 的用例漏 `throws SQLException`）；② ZSetCacheTest 1 例误对 `jedis` 校验窗口 `zrange`（实际发在 Pipeline 上）——修后全绿。**覆盖边界（显式声明）**：前端 `user.js` 改造**无自动化断言**（项目无 JS 测试），仅靠接口层 pytest + 人工路径确认；缓存侧三态/窗口/降级/回填均有单测。**红线合规**：未改 `@WebServlet` URL / web.xml / IoC 扫描 / 业务语义；未引入依赖（只用 Jedis 既有 ZSet 命令，签名已 `javap` 核对）；未动事务边界；缺省路径逐字节兼容（旧 pytest 用例零改动仍绿）；无 DDL。**G11 记录**：L1 ×3（前端落点错指 follow.js；BF 关注读路径图 follow TTL `10min`→实际 `cache.follow.ttlMinutes=30` 事实校正；`CURRENT_ARCHITECTURE` follow 域 `FollowCache（326）`→实测 **422** 行数滞后更正）、L2 ×1（验收"旧 pytest 用例不破"约束了缺省实现方式，已在计划与本文写明）、L3/L4 无。**subagent 独立评审**：见下条。
* **subagent 独立评审（2026-09-19，只读评审；结论与处置）**：**🔴 0 条，🟡 4 条，🟢 核心怀疑方向全部确认无问题**（缺省路径逐字节等价 / ZSet 升序≡`sortIds` 口径 / 窗口三路径同序同 total / DB 装载与判重只吃该页 / 事务边界未动 / 写路径 MULTI·探针·空标记·双 DEL·TTL 语义全保留 / 前端状态机 / 测试非空转 / 包层无新环 / 文档行数逐个核对吻合）。**🟡 处置**：① `BUSINESS_FLOW` 写路径表仍写"MULTI SADD 双写" → 改 **ZADD**（补 score 说明）✓；② **输出顺序实际由 `UserDao.findUsersByIds` 决定且该查询无 `ORDER BY`**（HEAD 既有脆弱点，唯一调用方 FollowService）——而验收明确要求"顺序稳定"，原先只靠存储引擎默认序成立 → **修**：补 `ORDER BY id`，使"ZSet 升序切片"与"DB 返回序"同口径（行为等价：实测 `full_ids == sorted(full_ids)` 本就通过；CA 6.17 增"顺序保证"条，另 user 域表 `UserDao（225）`→实测 **258** 行系既有滞后，一并更正）✓；③ CA 6.17 "一趟 pipeline"收紧为"**窗口取数**一趟 pipeline（探针一趟另行，与 SetCache 同模式）"✓；④ 前端"加载更多"无在途锁（仅按钮 disabled 防重、失败重试不重渲染）→ **不修**：风险低且 disabled 已覆盖连点主路径，真正的在途锁/并发预加载属"后端大分页 + 前端预加载"目标，另立项时一并设计。**处置后复跑**：JUnit 472 / 0 / 0 / 0、pytest 135 passed、exit=0（见"验证"）。**无法验证项（如实登记）**：真实 Redis/Tomcat 运行期行为（多页真实顺序、TTL 常量）只能由端到端 pytest 间接覆盖；Jedis 侧命令签名已用 `javap` 核对（`zadd(String,double,String)` / `zscore→Double` / `zrange→List<String>` / `zcard→long`，pipeline 与 MULTI 同由 `PipeliningBase` 提供）。
...
* **执行回写（2026-09-19，feat(prep-08) 已落地）**：**强制探索结论**：N11b/U-13 证据复核成立（`CommentController` L82 整树渲染、`CommentCache.loadCommentTree` L99-114 + `buildCommentTree` L117-145、`ContentService.getCommentsForContent` L114 无分页——三处逐行实测吻合）；**入口线索事实修正（L1）**：TASKS 原文把落点写在 `CommentController.java`，**接口实现实在 content 域 `ContentService` L114**（签名 `(long, Long)`），`CommentService` 只提供 `convertToCommentVOList` 转换 → 落点 = ContentService（切片 + 信封）+ CommentController（判参分支）；**有利发现**：`CommentDao.getComments` SQL 已带 `ORDER BY c.comment_id`，主楼顺序天然稳定，**不需要**像 T7 那样补 `ORDER BY`（T7 踩过的坑此处不存在）。**选型（用户 2026-09-19 拍板 D1=A + Q1~Q4 按建议）**：① **D1 = A「主楼分页 + 楼中楼整树」**（切片单位 = 主楼，children 整树随行；否决 B「楼中楼也分页」= 请求次数变差且违反"不撕裂楼中楼"、C「窗口读 / DB LIMIT」= 违反红线"不动缓存装载结构"、D「前端伪分片」= 响应体仍全量、治不了痛点）；② **D2 = 复用 content 域 `PageResult<CommentVO>`**（同域 → 零新类、无包层环；T7 因 content↔follow 跨域才自建 `FollowPageResult`）；③ **D3 = T7-B2 同款判参**（`hasPagingParams`；既有 pytest 按数组消费 `data or []`，缺省返回信封会直接打挂旧用例）；④ Q1 主楼保持 `comment_id` **升序**（= 现状）；⑤ Q2 `total` = **主楼条数**；⑥ Q3 前端头部计数改用详情接口 `commentCount`；⑦ Q4 读放大残留登记 **U-20**。**落地（代码/前端 3 改 + 测试 1 改 1 新增；文档 6 改）**：① `ContentService`（445→**501**）新增 `getCommentsForContent(contentId, userId, page, pageSize)` → `PageResult<CommentVO>`，并抽出**唯一切片点** `sliceRoots(roots, page, pageSize)`（越界页返回空列表但信封保留真实 `total`；后续换"切片源"只改此方法），点赞批量查询**只对该页**（`collectCommentIds(pageRoots)`）——**缺省重载一行未动**；② `CommentController`（105→**124**）`showComment` 加 `hasPagingParams` 分支 + 私有判参方法（与 `FollowController` L63-65 同构）；③ 前端 `detail.js`（470→**517**）以 `CHUNK_SIZE=50` 主楼为一块加载（首屏第 1 页 + `#commentMore` 的「加载更多」追加下一块，复用既有 `.load-more-btn` 样式与 user.js 同构模式），发/删评论后回到第 1 页重新累积，头部计数改 `state.content.commentCount`、评论区关闭时清空加载更多；④ 测试：`ContentServiceTest` **+10 例**（首页信封字段/末页余数/越界页保留 total/页间不重不漏且顺序一致/楼中楼整树随行/空树信封/关闭评论区信封/只对该页求 liked（`never` 全树）/无 user 跳过点赞查询/**非法分页参数兜底不抛异常**），新增 `src/test/python/test_comment_paging.py` **7 例**（自建专用内容使 total 可控：信封字段/缺省仍是全量数组/页间主楼前缀一致/半参数/越界保留 total/pageSize cap 50/楼中楼整体随行）。**验证（沙箱外，只信落盘证据）**：`python tools\tv.py test junit` → `latest.json` **482 / 0 / 0 / 0**（472→+10，`ContentServiceTest` 48→58）、exit=0、build_ok=true ✓；`python tools\tv.py test all` → pytest **142 passed**（135→+7）27.64s、exit=0、`build_ok=true`、`port_18080_open_after=false`、war 生成 ✓。**评审处置后（`sliceRoots` 加非法参数兜底 + 补 1 例）在最终代码状态复跑**：`tv.py test junit` **482 / 0 / 0 / 0**、`tv.py test all` pytest **142 passed**（28.18s），均 exit=0、`build_ok=true`、`port_18080_open_after=false` ✓。**红线合规**：`CommentCache`（`loadCommentTree`/`buildCommentTree`）**零改动**、缺省重载零改动（缺省行为逐字节一致）、楼中楼不切不撕裂、未改 `@WebServlet` URL/web.xml/IoC、未引入依赖；G9 本任务无 DDL。**覆盖边界（显式声明）**：前端 `detail.js` 的分块加载与「加载更多」**无自动化断言**（项目无 JS 测试，与 T7 同）；"缓存整树未被改动"由文件零 diff 保证。**G11 记录**：L1 ×1（入口线索落点修正，见上），L2/L3/L4 无。
* **subagent 独立评审（2026-09-19，只读评审；结论与处置）**：**🔴 0 条，🟡 1 条，其余怀疑方向全部 🟢**。评审确认：缺省重载经 `git diff` 逐行核对**零改动**（前置判断 / 空树 / 点赞组装与缺省路径逐字一致）；切片边界正确（`page=1`、末页余数、越界页空列表但保留真实 total、`total=0` 时 `totalPages=0`）；`collectCommentIds` 递归 children → **页内楼中楼的 `isLiked` 不丢**、未登录仍跳过点赞查询；`new ArrayList<>(subList)` 规避视图污染、楼中楼整树随行；`CommentCache` **零 diff**（红线①满足）；新增 import 仅同域 `PageResult`，未加重 content↔comment 既有包层环；JUnit 用例含 `verify`（只该页 / 未登录 never / 关闭评论区 never `getCommentTree`）与数字断言、pytest 用 module 级自建专用内容隔离充分（非空转）；前端状态机（累积 / 重置 / 按钮显示条件 / 关闭评论区清空 / `request` 消费 `body.data`）正确，`countTotal` 作 `commentCount` 缺失时的 fallback 非死代码；文档行数与 `wc -l` 吻合、用例数与 `latest.json` 吻合、L1 记录属实。**🟡 处置（1/1 已修）**：评审指出 `sliceRoots` 未防御非法分页参数（`page≤0` 时 `offset` 为负 → `subList` 抛 `IndexOutOfBoundsException`）——唯一生产入口 `CommentController` 已归一（`parsePage≥1`、`parsePageSize` 1~50），实际风险低，但**已采纳加固**：`sliceRoots` 对 `page<1 || pageSize<1` 返回空页（不假装第 1 页），**合法入参行为不变**（该分支在归一后的入参下不可达），并补 1 例 JUnit 覆盖该兜底。**无法验证项（如实登记）**：真实 Redis/Tomcat 运行期行为（多页真实顺序、TTL 常量）仅由端到端 pytest 间接覆盖；前端分块加载与「加载更多」无自动化断言（已在"覆盖边界"声明）。
