...
> 版本：2.8
> 最后更新：2026-09-27（feed2-21 T21 落表与写侧改造：§6.2 改写为「二期写侧：落表真相 + 写后失效 + 大V占位路由」——`feed_inbox` 表 = 时间线真相源、fanout 单写 DB + DEL 缓存；`/feed` 对外流程仍为 6.1 拉模式，**零变化**。上一版 2.7 = 2026-09-26 feed1-20 T20 收尾：新增 §6.2 写扩散与收件箱重建（影子期只写不读）。历史变更见 git 提交历史与 `NEXT_CYCLE_TASKS.md` 执行回写）
> 用途：保障重构时不破坏业务逻辑
...
> 读路径（feed2-21 T21）：仍为本节**纯拉模式**（`/feed` 对外零变化）；二期"切读"（收件箱窗口 + 大V发件箱两路读）属 **T23**。
...
### 6.2 写扩散与收件箱落库（二期写侧：落表真相 + 写后失效 + 大V占位路由）

> feed 二期「切读」的**写侧**（feed2-21 T21 落地）。**对外行为零变化**：`/feed` 仍走 6.1 的拉模式（读路径切换属 T23）；本节描述写侧如何维护 **DB 时间线真相源**与可降级缓存。

三层机制（都对业务链路**只降级、不抛出**；MQ 不可用不得阻断应用启动）：

```
发布（addVideo / addPost，事务提交后）
        │ publish feed.push.content
        ▼
   feed.push.queue ─► FeedPushConsumer ─► FeedBigVRouter.isBigV（大V占位路由：名单 OR 粉丝数≥阈值）
                                            │
                              ┌─ 大V ⇒ 跳过写扩散（内容由「大V发件箱」读时拉，T23）
                              └─ 普通作者 ⇒ 按粉丝窗口迭代（分页，200/批）
                                            ▼
                     ① INSERT IGNORE feed_inbox (user_id, content_id)   ← DB 真相（幂等）
                     ② DEL 收件箱缓存三件套 + DEL 完整态标记            ← 写后失效
                        （feed:inbox / empty: / partial: / feed:inbox:full）
...
| 机制 | 触发 | 产物 | 幂等 / 失败 |
|------|------|------|-------------|
| **写扩散 · 落库**（fanout，增量） | 发布视频 / 动态（**事务提交后**投 MQ） | `feed_inbox` 新增 `(粉丝, 内容)` 行（**DB 真相**）+ 该批粉丝的收件箱缓存失效（三件套 + 完整态标记） | `INSERT IGNORE` + `uk_user_content` 幂等（重复投递无副作用）；DB 失败**短路**本次 fanout；DEL 失败**不停写**（首次记日志后停用后续失效尝试）；**绝不抛穿发布接口** |
| **大V占位路由** | 同 fanout（判定单点，重建 / 读三处同源） | 命中 ⇒ **跳过写扩散**（不落库、不失效）；内容由「大V发件箱」读时拉 | 名单命中 OR 粉丝数 ≥ 阈值（`feed.bigv.userIds` / `feed.bigv.threshold`，**占位机制**：阈值波动 / 掉粉补推 / 名单维护归三期）；计数读取失败按普通作者处理（fail-open，宁可多写） |
| **收件箱重建**（rebuild，Redis 读缓存重算；T22 改窗口化 + 同步状态落表） | 关注 / 取关（**事务提交后**投 MQ，对象 = **发起方本人**） | 重建 `feed:inbox:{id}` 读缓存 + `feed:inbox:full:{id}` 完整态标记 | 三步 `DEL → DB 重查 → ZADD` 顺序**不可调换**；失败一律降级 ACK（仅载荷非法转死信）；`SET NX EX` 去重锁只是优化 |

- **分层（T21 起）**：`feed_inbox` 表 = **时间线真相源**（只存 DB 可重算的内容）；`feed:inbox:*` = **可降级读缓存**（TTL 仅为淘汰、无正确性含义）。写侧一致性 = **单写 DB 真相 + 写后失效 DEL**；**读 miss 回源 DB 并回填属 T23**。
- **窗口同步状态**：`feed_inbox_sync` 表（T21 建表、T22 起写入）——**存在即已同步**，替代一期"完整态标记"作读态闸门；一期 Redis 标记随 T22 收口退役（**T23 读态 / T24「未同步 → 回退纯拉」的落点**）。
- **过渡态（T21~T22 期间）**：Redis 里仍有 T19 重建产物、DB 表有 fanout 增量，二者并存且**无读方消费** ⇒ 对外无影响。
- **残余（已登记）**：写扩散失败 ⇒ 该批粉丝缺内容，由后续重建 / 丢消息兜底路径自愈（可靠性加固属三期）；fanout DEL 与重建交错（重建晚于 DEL 重写缓存）对外无影响（T23 读以 DB 同步态为准）。
- **二期切读门**：读路径切换（收件箱窗口 Redis→DB→回填 + 大V发件箱 + 归并去重截断 + total / 深翻契约）属 **T23**；影子核对工具 `tools/feed_shadow_check.py` 维持**只读**（窗口口径改写属 T25）。
