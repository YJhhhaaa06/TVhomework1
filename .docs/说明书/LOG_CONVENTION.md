...
| **存量分布**（2026-09-23 **T11 两批落地后**实测） | **159 个调用点 / 24 个持 `LOGGER` 的类**：SEVERE **63**（**T11 新增 19 处**，两批——第 1 次提交 11 处：事务模板包装点 1 + 改名级联包装点 1 + 评论链包装点 7 + 登录链包装点 2；第 2 次提交 8 处同族业务 wrap 缺口：`CommentService` 2 / `UserService.isAdmin` 1 / `FollowService.loadUserList` 1 / `CouponService` 4；全部在 catch 内且承担"唯一带堆栈"）+ WARNING **84**（79 处在 catch 内 = 78 处降级·自愈 + **T11 新增 1 处**（`CommentCache.ensureRootsWindow` 外层按异常类型拆 catch 后的 Redis 侧持栈行）+ 5 处非 catch 状态/统计）+ INFO **12**（基础设施 5 + T9 业务里程碑 7；T11 未改）+ FINE **0**。T6 复批结论见 TASKS T6 执行回写；T7 的双栈收口与 T11 的定栈口径见 3.1 附加纪律 2 与 3.4-③。**T8 增补**：审计记录器 `util/AuditLog` 的 INFO 写向 **audit 专属 logger（独立输出端）**、不进 `system.log` → 上面的分布不含它（见 3.5）。**T12 增补（2026-09-24）**：`UserService` 三个敏感变更方法**按异常类型拆 catch** → 调用点 159 → **162**（**+3** = 三个新 WARNING 分支；原 3 处 SEVERE 兜底**仍是调用点、计数不变**，只是覆盖面收窄）、SEVERE **63 → 63（不变）**、WARNING 84 → **87**（400/401/409 的失败路径由"SEVERE + 栈"改由新 WARNING 分支承载；级别语义修正、无新增能力）。**T14 增补（2026-09-25）**：`ContentCache` 索引懒重建链——**净零变化**（调用点 **162 → 162** / SEVERE **63** / WARNING **87**）：删 1 处（旧 `catch (Exception)` 的"索引重建 DB 装载异常"——其"返回空表"语义已废）、增 1 处（`ensureIndex` 的吸收结论行"索引重建装载失败（跳过重建，保留旧索引）"）；原"索引重建 DB 查询失败"包装点（WARNING + 栈）**保留原位**（T6 已判）。**T17 增补（2026-09-26，feed1-17）**：新增 `com.itheima.mq` 包（3 个持 `LOGGER` 的类）——调用点 **162 → 181**（**+19** = SEVERE **+1** / WARNING **+14** / INFO **+4**）、SEVERE **63 → 64**、WARNING **87 → 101**、INFO **12 → 16**、持 `LOGGER` 的类 **24 → 27**；另 FINE **0 → 2**（`MqConnectionManager` / `MqConsumerContainer` 的关闭路径诊断，低于阈值、不落盘）。**新增点的级别取向**：消费回调根捕获 = `SEVERE` + 栈（非 Web 线程唯一捕获点，见 3.1）；MQ 不可用 / 重连失败 / 发布未确认 / 死信投递失败 / 消费者启动失败 = `WARNING`（已被吸收的降级）；连接建立、重连成功、消费者启动、关停完成 = `INFO`（状态迁移） |
...
- **非 Web 线程必须自设"根捕获"**：`ExceptionFilter` 只覆盖 Web 线程；将来引入进程内线程池 / 定时任务后，**任务体最外层必须有 `try/catch` 记 `SEVERE`**（否则异常静默消失、无人知晓），并在提交前 `LogContext.wrap(task)`（见一⑦与 3.3）。**首个落地调用点（2026-09-26，feed1-17）**：`mq.MqConsumerContainer` 的消费回调——任务体最外层记 `SEVERE` + 栈后 `basicNack(requeue=false)` 转死信，**不套** `LogContext.wrap`；判定口径 = **"是否为本进程内提交任务"**（`wrap` 的语义是把提交线程的 reqId 透传给执行线程），而 MQ 消费是第三方库回调、无"提交"动作，且消息跨进程、生产者 reqId 无从传递 → `getRequestId()` 本就返回 null，套用属语义空操作。
- **`SEVERE` 与"是否吞掉"无关**（判据见 ② 第 ② 类）：存量 **9 处"吞掉型 SEVERE"**（MySQL 驱动加载 / 连接池初始化 / Bean `destroy`·`shutdown` / `ExceptionFilter` 自身 / 索引重建查询）**按"是否需要人介入"逐点判**——如驱动加载、连接池初始化失败 = 进程残废 → **保持 `SEVERE`**；关机期 `destroy`·`shutdown` 失败影响面小 → 可降 `WARNING`；**索引重建失败（查询/装载）= 缓存降级自愈路径、对外不 500（推荐降级为空）→ T6 判 `WARNING`**；**T14（log3-14）起装载失败不再当空表**（跳过重建、保留旧索引，见 `常青/CURRENT_ARCHITECTURE.md` 6.10/6.11），**级别零变化**。
