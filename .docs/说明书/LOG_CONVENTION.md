...
**存量未合规**：8 处"包装成 `DatabaseException`/`ServerException` 但不记日志"的业务 wrap 点（`CommentService` 2 / `UserService.isAdmin` 1 /
`FollowService.loadUserList` 1 / `CouponService` 4）待 **T11 commit 2** 收口，明细见 `目标与任务/NEXT_CYCLE_TASKS.md` T11 执行回写二节。
