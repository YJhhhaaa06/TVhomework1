...
| **存量分布**（2026-09-22 实测 + **T6 复批后** + **T9 补点后**） | T9 后 **139 个调用点 / 23 个持 `LOGGER` 的类**：SEVERE **44**（40 处上抛、**4 处吞掉但需人介入**保持）+ WARNING **83**（78 + 5 处降级并入，48 处为降级·自愈路径）/ INFO **12**（原 5 处基础设施日志 + **T9 新增 7 处业务成功路径**）/ FINE **0**（原 2 处死代码已删）。T6 复批结论：**原 9 处吞掉型 SEVERE** 见 `目标与任务/NEXT_CYCLE_TASKS.md` T6 执行回写（T6 只动级别，未改文案/控制流）。**T8 增补**：审计记录器 `util/AuditLog` 新增 1 处 `INFO` 调用点，但它写向 **audit 专属 logger（独立输出端）**、不进 `system.log` → 上面的分布（`system` 侧）不含它，审计留痕单列（见 3.5）。**T9 补点**：业务成功路径 INFO 由 0 处增至 **7 处**（登录 / 注册 / 添加视频 / 添加动态 / 删除内容 / 关注 / 取关），逐条判定与不记清单见 3.2 与 TASKS T9 执行回写 |
