...
> 本节为 **D-5（已定）** 落地方案：JUnit=单元基准、pytest=端到端、.http 降级本地调试不纳入回归。所有新增/改动测试前先按 0.2 判据选层，避免三套体系重复覆盖（P1）。
...
| .http（`src/test/*.http`）  | **本地调试，非回归** | IDEA HTTP Client 手跑                                   | 无断言框架；仅开发期肉眼确认响应；**永不作为回归载体**                                                         |

### 0.2 去重判据（C1-C4，按行为形态单选）
...
| C3 | 只是开发期想肉眼确认响应？                                             | → .http（不占回归份额）                      |
| C4 | C1、C2 都命中？                                                | → **按行为拆分**，每条子断言重走 C1/C2 单选，分别进两份文件 |
