...
> **已归档（2026-09-17）**：本周期（第四期「缓存体系综合改造」，260917-cache-overhaul，T1~T8 全部完成，`refactor(cache-01)`~`refactor(cache-08)`）已结束，本文档随周期归档至 `archive/目标与任务/260917-cache-overhaul/`。本周期范围（4.3）内 9 项已全部落地；未消化事项（R-02/R-03/R-04 本体/R-05/R-11/R-12、D 方向 feed 流改造、留池的 U-11）待第五期立项时消化。本文档为静态存档，不再更新。
...
| ---- | ---- | ---- | ---- | ---- | ---- |
| R-01 | 关注/粉丝计数入缓存 | 需求（方案待拍板） | `260914/R-02`（溯源 `260913/O-9`） | **已纳入本周期（T6，2026-09-16 已完成）** | Profile 的 followCount/followerCount 是否一并入缓存：关系 Set 是成员、计数是独立 key；注意 SCARD 冷 set 返 0 的坑（证据复核 2026-09-15：`ProfileService.getProfile` 每次从 DB user 表读计数）。**2026-09-16 T6 已按用户拍板落地**：独立计数 key（`user:followCount`/`user:followerCount`，String int，Cache-Aside）+ SCARD 否决 + 条件增量 Lua，详见 4.0 T6 决策段 |
...
| R-06 | 批量续期测试覆盖缺口 | 测试 | `260914/R-09`（溯源 `260913/T9-Review①②`） | **已纳入本周期（T7，2026-09-16 已完成）** | ① `getBatch` 续期未覆盖 miss key 场景；② Like/Follow 批量 pipeline 续期未直接断言（单 key hit-data 已覆盖）。**2026-09-16 T7 已补齐（只补测试不改行为）**：CacheAsideTest 新增 getBatch miss key 续期（pipeline 入列无效果 + 回填 setex TTL + 清空标记）；SetCache 批量与 Like/Follow 域层批量三态（hit-data/hit-empty/miss）直接断言 pipeline `expire` + 空标记不续 |
...
| R-09 | 缓存 JSON 无格式版本 | 需求（条件触发） | `260914/N8` | **已纳入本周期（T7，2026-09-16 已完成）** | DTO 字段删/改名后旧条目反序列化失败且降级路径不写回（Jackson 未关 `FAIL_ON_UNKNOWN_PROPERTIES`，`CacheAside` 脏 JSON → DEGRADE 直接 loader）→ 这批 key 在各自 TTL 内每次读都走 DB；证据复核 2026-09-15：`JacksonCodec` MAPPER 未 disable 该 feature。**2026-09-16 T7 已落地**：MAPPER 追加 `.disable(FAIL_ON_UNKNOWN_PROPERTIES)`（保留 WRITE_DATES_AS_TIMESTAMPS，无版本机制），脏 JSON（含未知字段）成功反序列化、多余字段忽略 → 不再反复降级走 DB（有意的定向行为变化）+ 兼容单测锁定 |
...
**T6（`refactor(cache-06)`，2026-09-16 执行定稿；方案 2026-09-16 用户三连拍板：① 独立计数 key ② SCARD 否决 ③ 条件增量；G11 质疑用户裁决照做）**：关注/粉丝计数入缓存（R-01）。**取向**：Profile 读路径 followCount/followerCount 由「随 user 整行从 DB 读取」改为**独立计数 key 入缓存**（`user:followCount:{userId}` / `user:followerCount:{userId}`，String int，Cache-Aside，0 合法，与 `content:likeCount` 同构），二次访问命中可观察；DB 仍为最终真理。**SCARD 冷 set 返 0 坑结论 = 否决 SCARD**——① miss/冷 set 返 0 且不触发装载（DB 有值时显示错误 0）② set TTL 过期返 0 ③ 计数与成员加载状态/TTL 耦合；成员（who）与计数（how many）分离（4.6 先例）。**关键实现点**：`CacheKeys` 新增 `userFollowCount`/`userFollowerCount`、`domainOf` 无需扩展（`user:` 兜底归 FOLLOW，T8 巡检项放行）；`UserDao` 新增 `getFollowCountById/getFollowerCountById`；`FollowCache` 注入 UserDao，读 `getFollowCount/getFollowerCount`（`cacheAside.get` + 单列 loader，镜像 getLikeCount 防御分支），写 `adjustCountsQuietly`（DB 提交后 `FOLLOW_COUNT_ADJUST_SCRIPT` = `exists 才 INCRBY delta`，KEYS=[两次计数 key]、ARGV=[±1]；冷 key no-op 由读回填；EVAL 失败→失效两计数 key 读自愈+WRITE_FAIL，不抛出）；`cacheFollow` → +1、`cacheUnfollow` → -1，成员 MULTI 双写与计数增量为两次独立调用、失败隔离；`ProfileService.getProfile` 事务外读两计数、不再消费行内字段。TTL 沿用 `cache.follow.ttlMinutes`（30min），app.properties 零改动。**G11 质疑（L3 上报，用户裁决「照做」）**：getProfile 必须读 user 整行（username 等），计数缓存**不减少任何 DB 查询**、冷 key 首访多 1~2 次单列计数查询——用户接受，读路径照走缓存（验收锚=篡改探针可观察命中）。**L1 观察 4 条（登记不修，见任务清单 T6 回写）**：404 多打 loader / DB 挂文案漂移（同 like 域先例）/ INCR 瞬时多计上限（like 4.6 先例内，TTL 自愈）/ DECR 理论负（miss 即纠）。**红线遵守**：对外行为零变化（计数数值/API 不变）、@WebServlet/web.xml/IoC 零改动、无 DDL、无新依赖。**验证**：JUnit **416 例全绿**（surefire 412 + pool 4，T5 407 + 9）+ pytest all **124 passed** + 运行时验证 8/8（`temp_script/verify_t6_count_cache.py`：冷 key 回填一致 / 条件 INCRBY 同步 +1 / **篡改探针 SET=999 → API 返回 999 ≠ DB 证命中缓存** / DEL 自愈 / unfollow 复原 / 冷 key no-op 读回填）+ subagent review 通过（无🔴，🟡5 全处置）。
...
**T7（`refactor(cache-07)`，2026-09-16 执行定稿；无方案拍板点）**：小项打包——JSON 兼容（R-09）+ 批量续期补测（R-06）。**取向**：① R-09 = `cache/JacksonCodec` MAPPER 追加 `.disable(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES)`（一行修，保留 `SerializationFeature.WRITE_DATES_AS_TIMESTAMPS` 原样，**不做格式版本机制**）——DTO 删/改名后旧缓存 JSON 含未知字段仍可反序列化（多余字段忽略），不再反复 DEGRADE 走 DB；此为**有意的定向行为变化**（脏 JSON 由「反序列化失败→DEGRADE→DB loader」变「成功解析→HIT_DATA」，正是 R-09 目标），对外 API/业务语义零变化；控制器层 `BaseServletUtil`/`BaseServlet` 自持 ObjectMapper 不碰。② R-06 = 只补测试断言不改行为：`CacheAside.getBatch` 补 miss key 续期场景（miss key 亦无条件入列 `p.expire`（返回 0 无效果）+ miss 回填 `writeOrInvalidate` setex 真实 TTL + 清空标记）；`SetCache` 批量（`batchIsMember`/`batchKeysIsMember`）与 Like/Follow 域层批量（`batchIsXxxLiked`/`batchIsFollowing`）补 pipeline `expire` 直接断言——批量三态（hit-data/hit-empty/miss）全部覆盖 + 空标记不续（SetCache 探针续期精确 TTL 无抖动，断言 `AppConfig.getLike/getFollowTtlSeconds()` 同源；CacheAside 批量续期 ±10% 抖动，断言 longThat(90..110)）。**红线遵守**：R-06 只补测试、无版本号/迁移逻辑、@WebServlet/web.xml/IoC 零改动、无 DDL、无新依赖。**验证**：JUnit **419 例全绿**（surefire 415 + pool 4，T6 416 + 3：JacksonCodecTest +2 / CacheAsideTest +1）+ **pytest all 124 passed 无回归** + subagent review 通过（无🔴，🟡2 全落实：batchKeysIsMember 混合态补 k1 断言 / 文件末尾换行）。

**T8（`refactor(cache-08)`，2026-09-16 执行定稿；收尾登记，无方案拍板点）**：全周期收尾——巡检 + 回归 + 文档同步 + 覆盖率。**残留巡检结论**：T1~T7 无残留——旧重复实现均已随 T2/T3/T4 删除（命中仅余 SetCache 自身方法与 javadoc 说明）；无 TODO/FIXME/临时开关/临时日志；`batchKeysIsMember` 生产无调用仅测试保留（T4 停用预留落实）。**原样校验**：@WebServlet 14 URL / web.xml / IoC 扫描（`scan("com.itheima")`）全部原样。**CacheStats 口径**：`domainOf` 对全 key 工厂分支逐一核对无漂移（T4 user:like*/user:commentLike* 于 user:* 兜底前归 LIKE、T6 计数 key 经 user:* 兜底归 FOLLOW，长前缀优先顺序成立）。**验证**：JUnit **419 例全绿**（surefire 415 + pool 4）+ **pytest all 124 passed**（本周期收尾全量回归点）+ 覆盖率地图 rerun **41 端点全有 pytest** 无回归。**文档**：常青 2.23 + BUSINESS_FLOW 3.1 核对一致。**G11**：无 L2/L3（T2 遗留 header 版本滞后 L1 观察随版本链自然对齐）。

**质疑记录**：T2 窗口 L1 仅记录 2 条（已载入任务清单 T2 执行回写，不打断）：① 日志标签 `contentId=`/`commentId=` 统一为 `id=`（保留 内容/评论 词，非对外契约）；② 常青文档 header 版本号存在 T1 未 bump 的滞后（2.16 表行后 header 仍 2.14，T1 的 6.13 无对应 header/12 行）——已随 T2 bump 至 2.17 并登记观察，留 T8 收尾统一对齐。**T3 窗口 L1 仅记录 1 条**（已载入任务清单 T3 执行回写）：域类内部日志措辞（"关注状态缓存读失败"等）统一为 SetCache 措辞，非对外契约（T2 同先例，无 L2/L3）。**T4 窗口 L1 仅记录 1 条**：`SetCache` 6.13 节注释残留旧 key 名「逐 contentLikeSet 判定」（组件零改动红线内），已在方法 javadoc 及节注释标注 T4 后该 API 停用预留，非对外契约——已随 review 🟡 落实。**T5 窗口 L1 仅记录 1 条（登记不修，超范围）**：`loadAllWithoutMedia` DB 装载失败返回空列表 → `rebuildIndexes(空)` 在 Redis 正常时会 SCAN+DEL 全量索引（既有行为——DB 瞬断清索引；零行为变化红线不随 T5 改动，留观察）；**反向边同登记**：空库（DB 无内容）时重建"成功"不记冷却 → 冷却对空库场景不生效，每请求仍全表查询（空表开销可忽略、非停机语义，留观察）。**T6 窗口 L1 仅记录 4 条（登记不修）**：① `getProfile` 用户不存在（404）场景多打计数 loader 查询（原不查）；② DB 全挂时错误文案由"获取用户主页失败"变"服务器异常，查询关注数/粉丝数失败"（同 like 域先例，均 500 无 API 契约）；③ 并发"DB 提交后→INCRBY 前"读 miss 回填已含 +1 真值再 INCR → 瞬时多计（like 4.6 条件写先例内已知上限，TTL 自愈，与 like 域同档风险）；④ DECR 理论负值（仅缓存值已低于 DB 真值才可能，miss 即纠）。**T6 窗口 G11 质疑（L3 上报，用户 2026-09-16 裁决「照做」）**：触发：动手前探索（R-01 证据复核）；证据：`getProfile` 必须读 user 整行（username 等，`select *`），计数随行免费携带，独立计数 key **不减少任何 DB 查询**、冷 key 首访反而多 1~2 次单列计数查询；主张：必要性质疑（"减少 DB 读"的收益假设不成立）；建议：降级留池 or 收缩仅写路径；用户裁决：**照做落地**——唯一消费方（Profile）仍按「读路径命中缓存」落地（验收锚=篡改探针可观察命中），接受冷首访 loader 冗余查询作为代价。（记录入任务清单 T6 执行回写 + 本档 4.0 T6 段）
...
| 日期 | 版本 | 内容 |
| ---- | ---- | ---- |
| 2026-09-16 | 0.9 | **T8 执行定稿回写**：4.0 追加 T8 收尾登记（全周期收尾——残留巡检 T1~T7 无残留 / @WebServlet 14 URL+web.xml+IoC 原样 / CacheStats domainOf 全 key 分支核对无漂移 / JUnit 419 + pytest all 124 + 覆盖率地图 41 端点无回归 / 常青 2.23 + BUSINESS_FLOW 核对一致 / 无 L2/L3） |
| 2026-09-16 | 0.8 | **T7 执行定稿回写**：4.0 追加 T7 技术决策（R-09 JacksonCodec 关 `FAIL_ON_UNKNOWN_PROPERTIES` 一行修 + R-06 批量续期断言补齐——getBatch miss key 场景 + SetCache/Like/Follow 批量三态 pipeline expire 直接断言 + 空标记不续；脏 JSON 路径有意定向行为变化说明；JUnit 419 / pytest 124）+ 二表 R-06/R-09 状态列 →「已纳入本周期（T7，2026-09-16 已完成）」并补落地结论 |
| 2026-09-16 | 0.7 | **T6 执行定稿回写**：4.0 追加 T6 技术决策（R-01 计数入缓存——2026-09-16 用户三连拍板：独立计数 key+Cache-Aside / SCARD 否决 / 条件增量 Lua；G11 质疑 L3 上报用户裁决「照做」；JUnit 416 / pytest 124 / 运行时验证 8/8）+ T6 窗口 L1 质疑记录 4 条 + G11 质疑记录段（必要性存疑→用户裁决照做）+ 二表 R-01 状态列 →「已纳入本周期（T6，2026-09-16 已完成）」并补拍板落地结论 |
