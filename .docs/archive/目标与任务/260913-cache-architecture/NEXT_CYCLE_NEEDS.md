...
> **已归档（2026-09-13）**：本周期为 C 方向缓存改造（缓存体系重制为统一 Redis：统一 Redis + 三态 Cache-Aside + 空标记独立 key + 可降级 + 统一单飞 + 点赞计数/成员分离 + 关注双 Set；一版 T1~T6 打地基，二期 T7 观测埋点 / T8 读路径加固 / T9 TTL 精调，共 9 个 commit `refactor(cache-01)`~`refactor(cache-09)`，对外行为零变化）。后续周期文档见 `.docs/目标与任务/`（勿改此归档文件，仅追溯用）。
