...
## 三、任务总览（本周期 14 任务：T10-A/T10-B、T11-A/T11-B/T11-C、T12~T19（均已完成）、T20 预告；**T10/T11 拆解后计数**——原"12 任务"是按 T10 未拆时写的，编号未变）
...
| T11-A | 关注/粉丝列表：域级上限 200 + 后端固定信封（前端只传 `page`）+ 缺省归一为第一页 + sheet 接 `chunkedList` | NEEDS 4.2 ②（+ N15 复用） | 无 | **已完成（2026-09-20）**：`pageSize=51` 回显 51、`999`→200；缺省=第一页信封（与显式 `page=1&pageSize=200` 逐字节一致）；sheet「加载更多」走公共 helper；全链 exit 0 + JUnit 482/0/0/0 + pytest 145 | `refactor(cache-11a)` | **已完成**（其中"上限 200"已于 **T19** 调整为 100，见 T19 行） |
...
| T17 | IoC 注入解析失败可观测（消除"取不到就跳过"） | NEEDS 4.1 **N12**（前半；声明式事务不在本任务） | 无 | **已完成（2026-09-20）**：`@Inject` 取不到 Bean 逐条 WARNING + 未注入清单汇总（恒有）；`ioc.failFast` 默认 true 注入点即抛（用户裁决 fail-fast 更优）；`IocContainerTest` 3 用例可复现验证（fail-fast 开/关/成功路径）；全链 exit 0 + JUnit 514/0/0/0 + pytest 145 + 启动无新增失败；subagent review APPROVE | `fix(ioc)` | **已完成** |
| T18 | 配置卫生：本机绝对路径可外部覆盖 + `AppConfig` 带默认值读取 | NEEDS 4.1 **N13**（分页上限参数化已随 T10-B/T11-A 落地，本任务不含） | 无 | **已完成（2026-09-20）**：`context.xml` `/upload` base 改 `${upload.path:-默认}` 系统属性占位符（Tomcat 原生替换，默认值不变）；`AppConfig` 新增系统属性覆盖通道（环境变量 > `-Dkey` > app.properties）+ 带默认值读取重载；`-Dupload.path` 外部覆盖沙箱闭环验证 PASS（启动校验通过 + 上传落盘覆盖目录 + `/upload` 静态 200）；默认行为由 JUnit 默认值用例 + 全量回归兜底；全链 exit 0 + JUnit 522/0/0/0 + pytest 145 | `chore` | **已完成** |
| T19 | feed / search / profile 的后端大分页（域级上限 + 域级信封）→ **执行中扩为四域（含 follow 200→100）** | **2026-09-20 T11 窗口推后项** | T11-B（三处前端已迁 helper） | **已完成（2026-09-21）**：四域 `pageSize` 缺省/上限均 **100**（follow 由 200 降 100；评论域不动）；三域只传 `page` 返回域级信封、前端 4 文件去 `pageSize` 且 chunk 100；`BaseServletUtil` 归一化下沉（`normalizePage`/`normalizePageSize`）+ 删单参/两参重载与公共 cap 50；JUnit 538/0/0/0（+12）+ pytest 161/0/0/0（+16）+ 全链 exit 0 | `refactor(cache-19)` | **已完成** |
| T20 | 评论域后续大改（可能含热度排序） | **用户 2026-09-21 拍板"comment 域后续开任务大改"**（关联 NEEDS 4.1 **N17** 评论热门排序） | 无（待立项时盘点；可能有前置拍板） | **预告条目**：四项要素留待执行窗口调查后拍板并回写（例外条款同 T10/T11/T19）；**前置拍板项 = 是否放开"不引入 MQ"红线**（热度排序若走异步；否则改走进程内线程池），以及是否采用 N17 的最小可行替代（`order=hot` 排序参数化） | 待定 | **预告** |
...
* **执行回写（2026-09-20 T17 窗口）**：**用户裁决 = WARNING + 可配置 fail-fast（`ioc.failFast` 默认 true）**（G7；用户确认「成员变量注入失败服务无法正常运行，快速失败更好」；静态核对 14 个 Controller 共 18 个 `@Inject` 字段全部对得上 `@Component` 注册 Bean、**无存量静默未注入实例** → 默认 fail-fast 不破坏启动）。实现：`injectFields` 改为收集 `MissingDependency` 返回 → `reportMissing` 逐条 WARNING + 未注入清单汇总（恒有，`IocContainer.java`），fail-fast 时注入点抛 `IllegalStateException`（Bean 在 context 启动注入失败 = 启动失败；Controller 懒加载在首请求 servlet init 失败——servlet 生命周期决定、不改）；`@InjectConstructor` 路径本就 fail-fast（`IocContainer.java:100-103`）未动；`AppConfig.getIocFailFast` + `app.properties ioc.failFast=true`（环境变量 `IOC_FAILFAST` 可覆盖）；新增 `IocContainerTest` 3 用例（fail-fast 开 = 抛错 + 双日志前缀 + 字段 null / 关 = 仅告警不抛 / 成功路径 assertSame + 零 WARNING）支撑"可复现验证"验收。验证：全链 exit 0 + JUnit **514/0/0/0**（511 基线 + 3 新增）+ pytest **145/0/0/0** + 启动日志无新增失败（`IOC 容器初始化完成` 正常、无注入 WARNING/SEVERE）；subagent 独立 review **APPROVE**（仅 1 条建议级注释措辞已修）。改动：`IocContainer`/`AppConfig`/`app.properties`/`IocContainerTest` + 常青 CA §4.2。
...
* **执行回写（2026-09-20 T18 窗口）**：**N13 证据复核成立**——`app.properties:32` 本机绝对路径随包入库、`AppConfig` 逐键 getter 属实、"换环境不可用"由 `AppShutDownListener` 强校验（context.xml base == upload.path，不一致拒绝启动）实锤（只设 `UPLOAD_PATH` 环境变量时 context.xml 打包值 ≠ 覆盖值 → 启动必失败）。**决策（拍板口径）**：① 外部覆盖统一通道 = **JVM 系统属性 `-Dupload.path`**（Tomcat 对 context.xml `${upload.path:-默认}` 占位符原生替换——SystemPropertySource 始终启用 + `${name:-default}` 默认值语法，官方文档背书；AppConfig 同步读系统属性，**两侧同源天然一致**；换环境只改 `CATALINA_OPTS`/setenv，不改源码/不重打包）；② 默认值不变（`app.properties` 与占位符默认值同为 `D:/data/projects/VideoPlatform/stone`，**两处须保持同步**，已写进常青 CA §5.1.1 与 context.xml 注释）；③ `UPLOAD_PATH` 环境变量通道**保留**（run_tests 依赖，优先级 env > sysprop > app.properties），单独使用仍会与 context.xml 校验冲突（与 T18 前行为一致，非回归）。**实现**：`context.xml` base → `${upload.path:-D:/data/projects/VideoPlatform/stone}`；`AppConfig.load()` 环境变量查不到时 fallback `System.getProperty(key)`；`AppConfig` 新增包可见带默认值重载 `get/getInt/getLong/getBoolean(key, def)`（键缺失/空 → 默认值；数值解析失败照旧抛 fail-fast；**既有 getter 语义零变化**）；`AppShutDownListener` 新增 `resolvePlaceholder`（模拟 Tomcat 展开：sysprop > 占位符默认，**不查环境变量**），校验语义 = "Tomcat 实际挂载 == AppConfig 实际使用"。**验证**：新增 `AppConfigTest`（默认行为同源比对 / 系统属性覆盖反射重建 PROPS / 带默认值 5 类用例 / 解析失败照旧抛）+ `AppShutDownListenerTest`（resolvePlaceholder 6 用例）；沙箱闭环脚本 `temp_script/t18_verify_upload_override.py`：**不 patch context.xml**（保留占位符）+ `CATALINA_OPTS=-Dupload.path=<verify-media>` 启动 → 启动成功（校验通过）→ 注册/登录/上传（video+cover）→ 落盘 verify-media → `/upload/video/<file>` 静态 200 → **PASS**；全链 `python tools\tv.py test all` exit 0 + JUnit **522/0/0/0**（514 基线 + 12 新增）+ pytest **145/0/0/0**。改动 4 main + 2 test + 1 验证脚本 + 常青 CA §5.1.1/目录树。**一次性验证脚本已留 `temp_script/`（G10）**。**subagent 独立 review APPROVE**（9 项全 PASS、零阻断；采纳 2 条建议：① `AppShutDownListener.validateUploadPath` 对空 `upload.path`（如 `-Dupload.path=` 空覆盖）拒绝启动；② 验证脚本 `upload_and_verify` 异常路径 try/finally 兜底停服；修复后全链重跑 exit 0 + pytest 145 全绿）。
...
* **强制探索步骤**：动刀前先 (0) 复核三处 Controller 上限现状（公共 50）与前端 chunk 现状（T11-B 后为 50） (1) 定各域限额（建议 `200/200`；**窗口内用户拍板改为 100/100**）与前端 `batch` 的匹配 (2) 确认 `publish.js` 与 `user.js` 共用 `/profile` 的一致性（一次改两处消费） (3) 若清单未覆盖 → 回写本文档再动手。
* **验收**：三域 `pageSize` 上限生效、只传 `page` 返回域级信封；前端四处改为只传 `page` 且 chunk 上调；相关 pytest 全绿（如有上限断言同步改写）；全链 exit 0。

* **窗口内拍板（2026-09-21，用户；G7 回写 NEEDS 4.0）**：① 信封取值 **100**（原拟 200，用户判定"200 太高"）② **不后端硬忽略 `pageSize`**（前端不传即可；硬忽略只会封死三域"页间不重不漏"的验证路径，收益仅是语义观感）③ **follow 域同批 200 → 100**（四域同口径）④ **comment 域不动**（用户："后续开任务大改"，可能含热度排序）⑤ search 解析点改造 + 公共解析遗留清理 + 新增 `BaseServletUtilTest` + 附加发现登记留池：均按执行侧建议（Q3~Q9）执行。
* **执行回写（2026-09-21 T19 窗口）**：
  - **落地**：`BaseServletUtil` 把归一逻辑下沉为 **request 无关纯函数** `normalizePage(Integer)` / `normalizePageSize(Integer,max,defaultSize)`，`parsePage` 与三参 `parsePageSize` 委托二者；**删除**无参 / 两参重载与 `DEFAULT_PAGE_SIZE_MAX` / `DEFAULT_PAGE_SIZE`（两参重载自 T11-B 起即已零调用）。四域常量：feed / profile / search 各新增 `XXX_PAGE_SIZE_MAX/DEFAULT = 100`，follow 由 200 → 100；`SearchController.search` 的 GET 分支与 JSON body 分支**收敛到同一归一出口**（顺带消除 `page=abc` / `pageSize=-1` 的旧 500）。前端 4 文件（`follow.js` / `search.js` / `publish.js` / `user.js`）**去掉 `pageSize` 只传 `page`**，chunk 50 → 100（`user.js` 的 sheet 常量 200 → 100）；`chunkedList.js` **零改动**（自适应 + "本地余量用尽才拉"本就满足需求，未做预取）。
  - **验证**：JUnit **538/0/0/0**（T18 基线 522 + 新增 `BaseServletUtilTest` **12** 用例；surefire 逐类求和 = `latest.json` 一致）+ pytest **161/0/0/0**（145 基线 + 新增 `test_content_paging.py` **16** 用例）+ 全链 `python tools\tv.py test all` **exit 0** + 前端 5 文件 `node --check` 语法通过（`.mjs` 副本，未落库）。
  - **红线合规**：分页语义 / 返回集 / 排序 / 跳过 null 口径零变化；`ProfileVO{contentPage}` 形状不变；未动 `/feed` 关注 ids 全量读（R-01 保留）与 `/profile` 全量 id 读（另登记池 U-24）；未引入新依赖；**未删任何现有测试用例**（`test_follow_list.py` 的 200 断言逐条改写为 100，`page_size=1` 跨页与 51 回显用例保持不动）。
  - **G11 记录**：三条 L1——① 复核发现**两参 `parsePageSize(req,max)` 自 T11-B 起已是零调用死代码**（随本任务清理）；② `/search/keywordSearch` 的排序 `ORDER BY create_time DESC` **缺 id tie-breaker**（feed/profile 均有），页间漂移隐患且前端 `seen` 去重会掩盖 → 池 **U-25**；③ `/profile` 装载侧**全量 id 读 + 内存切片**与 U-18 同型未治 → 池 **U-24**。**修正一条既有口径**：NEEDS 4.3 原写"后端硬忽略 `pageSize` 会摧毁 pytest 小信封验证能力"——窗口内逐域复核为**分域成立**（对 follow/comment 成立；对 feed/search/profile 当前不成立，但那三域的跨页正确性本无自动化验证），已按此改写并请用户裁决（用户裁定：不硬忽略）。
  - **覆盖边界**：`/search/keywordSearch` 的 **JSON body 分支（POST 形态）无 pytest 覆盖**（前端只用 GET），由 `BaseServletUtilTest` 的 `normalize*` 用例 + 代码审查兜住；Controller 层无单测层（项目约定），端到端归一行为全部由新增 pytest 断言承载。
  - **评审（subagent 独立只读，2026-09-21）**：结论 **APPROVE with suggestions**。8 个怀疑方向逐条 🟢——语义等价性（`normalize*` 与原 `parsePage`/三参 `parsePageSize` 逐条等价，含"`parseInt` 不去空格"细节）/ 删除重载零调用（全仓含 `src/test` 无悬空引用）/ search 双分支同一出口（幂等）/ 前端无 `pageSize` 残留且 helper 末页判定与自适应成立 / 新增断言非空转（"缺省==显式"三条均有非空守卫；`test_follow_list.py` 仅改数值未放松）/ 文档行数与回写数字与实测一致 / 历史留痕与现行口径已逐处区分 / 红线合规（未删用例、未引依赖、未动 R-01 与全量 id 读路径）。**唯一 🟡 = "确认 search 信封 12→100 与隐式 cap 的产品接受度"** → 处置：**已由用户在本窗口拍板**（Q1 批准缺省 10/12 → 100），无需改动。评审另提示 1 条**既有边界**（非 T19 引入，L1 仅记录）：`SearchController` 以 `req.getContentLength() <= 0` 区分 GET 与 JSON body 分支，将来若前端以 chunked transfer encoding 发 JSON body 会被误判为 GET 分支——本任务不动。
