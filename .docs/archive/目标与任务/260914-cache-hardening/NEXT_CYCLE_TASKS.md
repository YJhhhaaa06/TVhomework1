...
> **已归档（2026-09-15）**：本周期（第三期「缓存加固」，260914-cache-hardening，T1~T6 全部完成，`fix(cache-01)`~`fix(cache-06)`，T4 拆 04a/04b/04c）已结束，本文档随周期归档至 `archive/目标与任务/260914-cache-hardening/`。本文档为静态存档，不再更新。
...
* **执行回写（2026-09-14，fix(cache-03) 已落地）**：
  * **强制探索结论**：① 全部 `return null` 逐一确认——T3 治理面仅 `ContentCache.loadContentFromDb` + `CommentCache.loadCommentTree` 两个 loader（其余为 DAO 无行=确认无数据，或 `findContentIdByCommentId` 非 CacheAside loader，不喂 markEmpty）；② 对外结果现状：内容 loader 失败→404、评论→空、批量→跳过；like/follow loader 失败→500（抛 ServerException）；③ **区分机制（用户 2026-09-14 拍板=对外行为保持）**：loader 失败抛 `DatabaseException`（事务模板已把 SQLException 包成它，现成 500 语义载体），CacheAside 在各装载点捕获后转 null——**不写空标记、不 DEL 数据 key**；CacheAside 结构上已有"loader 抛异常天然不 markEmpty"契约（markEmpty 在 invokeLoader 之后），本次补的是显式捕获转 null，防批量读整请求 500。
  * **实现**：`ContentCache.loadContentFromDb` 重构（内层 `catch(SQLException)→null` 移除，SQLException 交由事务模板包成 DatabaseException；外层分流 DatabaseException 重抛 / NotFoundException·ServerException（媒体损坏·未知类型）→null / 意外 Exception→包 DatabaseException）；`addContent`/`refreshContent` 守卫（DB 提交后缓存同步遇 DatabaseException 静默跳过；refresh **保留旧缓存**不做删除语义，读自愈）；`CommentCache.loadCommentTree` 同构重构（空/无行→null 确认无评论，失败抛 DatabaseException）；`CacheAside` 5 个装载点捕获 DatabaseException→null（getInternal miss 内联 try/catch 直接 return null **跳过 markEmpty**；getInternal 降级 / getBatch 整批降级 / getBatch 脏 JSON 单 key 降级共用新增 `loadDegraded` helper；删死代码 `loadBatch`）；类 javadoc 补"loader 失败不得上报为空"契约（仅对 DatabaseException 生效，like/follow 的 ServerException 不受影响）。**红线对照**：三态语义/空标记机制/TTL/key 命名/对外错误约定零改动。
  * **验证**：`tv.py test junit` **348 例全绿**（surefire 344 + 独立 fork pool-test 4；T3 新增 7 例无删除：CacheAsideTest +5——miss/降级/批量 miss 逐 key/批量整批降级/批量脏 JSON 单 key 降级 失败均不写空标记不 DEL，ContentCacheTest +2——loader 抛 DatabaseException 与 addContent 静默跳过，CommentCacheTest 改 2 断言改 assertThrows）+ `tv.py test all` pytest **124 passed**；**独立 subagent 评审通过**（无🔴；🟡 4 条：批量降级两路径补 2 例已落实、测试 mock 未走真实事务模板包装=记录已知限制、loader 与 CacheAside 双日志级别=可接受、全限定名=已修）。
  * **验收对照**：DB 瞬时失败不写 60s 空标记 ✓（单测断言 never setex(empty:) / never del 数据 key）；既有数据 key 未被 DEL ✓；新增单测覆盖"加载失败不写空标记" ✓；mvn compile + JUnit 全绿 ✓；pytest 无回归 ✓。
...
* **执行回写（2026-09-14，fix(cache-04a) 已落地——① N3 markEmpty 空标记写入守卫）**：
  * **强制探索结论**：① `markEmpty` 全部调用方 5 处，均为"确认无数据"语义且都在单飞 leader 内——`CacheAside.getInternal` miss（L157）/ `getBatch` miss（L256）、`LikeCacheService.writeContentLikers`/`writeCommentLikers` 空分支（L499/L517，已走公共方法自动受益）、`FollowCache.writeSet` 空分支（L463-477，手写重复实现）；② 方案选择（用户 2026-09-14 拍板）：**exists 守卫**（对齐 `writeSet` 260913 先例）而非 Lua 原子化——Lua 需 RedisAccess 新增 eval 路径、mock 复杂化，而守卫已把竞态窗口从"loader 全程"缩到毫秒级；③ 原 `del(dataKey)` **随守卫移除**：守卫内为死代码（进入分支前提=数据 key 不存在），且现状竞态下它是 N3 危害的组成部分（删掉并发刚写入的真数据），`writeSet` 复用后先例守卫内的 del 一并消除。
  * **实现**：`CacheAside.markEmpty` 加 `if (!j.exists(dataKey))` 守卫 + 移除 `del(dataKey)`（javadoc 记录守卫语义、失败口径与残余竞态）；`FollowCache.writeSet` 空分支定向复用 `cacheAside.markEmpty(setKey)`（U-09 允许的 T4 定向复用，零构造器改动；非空分支/读路径/降级路径零改动）。**残余竞态（已接受）**：exists 检查→setex 的毫秒间隙内并发写入时空标记可能覆盖其上——数据 key 未被删，空标记 60s 过期或下次业务写 `writeOrInvalidate` 清空标记即自愈，无真数据丢失。
  * **验证**：`tv.py test junit` **351 例全绿**（T3 末 348 +3：CacheAsideTest 新增 markEmptySkipsWhenDataKeyExists / markEmptyExistsFailureSkipsQuietly（守卫检查失败=保守不写）/ concurrentBackfillAndWriteNoFakeEmpty（两线程确定性时序：B 写真数据完成后 A 的 loader 返回 null → markEmpty exists 守卫必见数据 key 存在 → 断言无空标记无 DEL；mock RedisAccess 直通共享 mock Jedis 规避 MockedStatic 线程局部限制））+ `tv.py test all` **pytest 124 passed** 无回归。
  * **测试适配（随 del 移除）**：`CacheAsideTest` 改 3 处断言（markEmptyWritesEmptyMarkerWhenDataKeyAbsent / getMissLoaderNullWritesEmptyMarker / getBatchMissNullLoaderWritesEmptyMarker——`verify del` 改 `never del`）；`FollowCacheTest` 改 3 处（isFollowingMissEmptyFollowsWritesEmptyMarker / getFollowingIdsMissEmptyWritesEmptyMarker / writeEmptyMarkerSkipsWhenSetAlreadyExists——空分支断言由 `verify(jedis).setex(empty:)` 改为 `verify(cacheAside).markEmpty(...)`，守卫行为断言内聚 CacheAsideTest）。
* **执行回写（2026-09-14，fix(cache-04b) 已落地——② N4 索引写失败自愈）**：
  * **强制探索结论**：① 缺陷链——`addToIndex` 失败仅记 WARNING（该内容不在任何 `content:index:*`），而 `ensureIndex` 只判"当前读的 indexKey 是否存在"，索引 key 存在则懒重建永不触发 → 内容长期不进推荐，只能等重启 init；② 自愈基建已在：懒重建（`ensureIndex` 索引 key 缺失 → `singleFlight.get(INDEX_REBUILD_KEY)` 单飞全量 `loadAllWithoutMedia`+`rebuildIndexes`）——缺的只是"让索引 key 回到缺失状态"的触发器；③ **方案（plan 定稿）**：catch 内 best-effort DEL 本内容所属 4 个索引 key（复用既有懒重建，**零新增 key**——否决脏标记 key（触碰 key 命名边界、T6 U-08 要管）与完整性校验（无便宜一致性信号、成本高）。
  * **失败场景三分收敛**：抖动已过 → catch 内 DEL 成功 → 下次读自愈；Redis 持续挂 → DEL 也失败不抛（读路径同样降级，与现状一致无新增伤害）；DEL 部分成功（命令序列中断）→ 已 DEL 的 key 缺失照样触发全量重建（rebuildIndexes 本身全量 DEL+重建）→ 收敛。
  * **实现**：提取 `indexKeysOf(type, categoryId)`（4 个索引 key 与 `lremAndLpush` 同源，防两处硬编码漂移）；`addToIndex` catch 改述日志 + `deleteIndexKeysQuietly`（双层 best-effort，DEL 也失败仅记日志）；类 javadoc 补自愈语义。**不动**：ensureIndex / rebuildIndexes / readIndex / addContent / refreshContent / removeContent 零改动；不引入 CacheStats 挂点（避免改构造器，与 removeContent 同口径，日志即检测）。
  * **验证**：`tv.py test junit` **353 例全绿**（04a 末 351 +2：addContentIndexWriteFailureDeletesIndexKeysForSelfHeal——写抛/DEL 直通差异化 stub 断言 4 key 各 DEL 一次且无 lpush，注意 `doNothing()` 会覆盖 setUp 直通 stub 须用 doAnswer 直通（实测修正）；addContentIndexWriteAndHealDeleteFailureDoesNotThrow——恒抛双层失败不抛）+ `tv.py test all` **pytest 124 passed** 无回归。
  * **残余窗口（已接受）**：Redis 持续挂时 DEL 也失败 → 恢复后索引仍可能不完整（与现状一致，读路径降级兜底）；懒重建触发面仍限 getRecommendByFilter 读路径（与现状一致，不做 R-10）。
* **执行回写（2026-09-14，fix(cache-04c) 已落地——③ N7 条件写 Lua 原子化，T4 关闭）**：
  * **强制探索结论**：① 治理面实为 **4 个方法**而非入口线索字面的 2 个——`unlikeContent`/`unlikeComment` 存在同款竞态（探 exists→DECR，并发失效后计数以 **-1** 重建），用户拍板对称孪生同修（T2 先例）；② 原子手段确认：Jedis 5.1.0 `eval(String, List<String>, List<String>)` **javap 实证存在**，经既有 `redis.executeVoid` lambda 调用，RedisAccess 零改动；③ 失败路径核实：`executeVoid` 委托 `execute`，一切 RuntimeException 统一包装 `CacheException` + 熔断成败回填——eval 异常/脚本错误走同一通道，**熔断口径与降级语义不变**；④ TTL 核对：4 方法现状均不设 EXPIRE（miss 回填维护），Lua 版保持；⑤ 测试现状：content 版写路径 4 测、**comment 版写路径零测试**（对称缺口，本次补齐）。
  * **方案（用户 2026-09-14 拍板：Lua 原子化 + 4 方法全治理）**：两个脚本常量（static final 包可见）——`LIKE_CONDITIONAL_SCRIPT`（KEYS=[setKey,countKey,emptySetKey]，ARGV=[userId]：DEL 空标记 + SADD/INCR-if-exists，与原两趟语义逐条一致且合并为一趟往返）、`UNLIKE_CONDITIONAL_SCRIPT`（KEYS=[setKey,countKey]：SREM/DECR-if-exists，保持"不清空标记"语义——空标记=确认无点赞者，unlike 不改变该事实）。4 方法体统一 `executeVoid(j -> j.eval(...))`，catch 降级（日志 + WRITE_FAIL + invalidate）不变。零新增 key（KEYS 均为既有 key）。
  * **验证**：`tv.py test junit` **356 例全绿**（04b 末 353 −1 +2 改 2 断言为 eval 参数断言（likeContent/unlikeContent RunsConditionalLuaScript）、删 likeContentSkipsWhenKeysAbsent（条件语义内聚脚本，mock 无法执行 Lua，独立价值消失）、增 likeComment/unlikeComment RunsConditionalLuaScript（对称补测）+2 评审补齐 unlikeContent/unlikeCommentFailureInvalidatesKeys 失效降级对称覆盖；likeContentFailureInvalidatesKeys 零改动——doThrow executeVoid 已覆盖 eval 路径）+ `tv.py test all` **pytest 124 passed** 无回归。**运行时 Lua 冒烟**（`temp_script/verify_cache04c_lua.py`，对真实 Redis EVAL）：5 场景 10 断言全过——like 命中（SADD+INCR 生效+清空标记）/ like 冷 key 不建残缺缓存 / unlike 命中（SREM+DECR 生效）/ unlike 冷 key 不建（防 -1 重建实证）/ unlike 保留空标记——补齐脚本文本零执行验证缺口（评审🟡落实）。**条件语义验证口径（已接受）**：单测验证调用参数（脚本 + KEYS/ARGV），运行时语义由冒烟脚本 + Redis 保证。
  * **验收对照**：条件写竞态有明确结论=修掉 ✓（Lua 原子化，非记录接受）；mvn compile + JUnit 全绿 ✓；pytest 无回归 ✓；零新增 key / 对外行为零变化 ✓。
...
* **执行回写（2026-09-14，fix(cache-05) 已落地）**：
  * **R-01 拍板（用户 2026-09-14）**：取向 = **全量 + 工程化优化**——保留"启动预加载全部内容+索引"语义（用户理由：个人项目流量小、全量在当前数据量无压力；未来数据量成瓶颈再另周期评估按需回填/分级加载，本期不预埋开关）。
  * **强制探索结论**：① `init()` 由 IocContainer `invokeInitializable` 启动调用（唯一 `Initializable` @Component），失败影响=外层 try/catch 记日志 return（缓存为空走读自愈，不 crash 应用）；② 现状量化——DB = 1（findAllContent）+ N（逐条 findMedia，N+1）；Redis ≈ 12N 往返（`writeContent` 每内容 setex+del 空标记 2 往返 + `rebuildIndexes` 每内容 4 key ×(lrem+lpush) 8 往返 + SCAN 页），全部串在一个长 DB 事务里（事务全程占连接）；③ R-01 三取向代价对比完成（全量=保留现语义最稳；按需回填=启动近乎瞬时但首访付 DB 装载成本、媒体损坏空标记语义略变；分级加载=折中需额外维护复杂度）→ 停决策点报备用户拍板 = 全量 + 工程化优化。
  * **实现**：`ContentCache.init()` 拆两段——DB 阶段 `transactionTemplate.execute(this::loadBuildableFromDb)` **事务内只读**（findAllContent + 新增 `ContentMediaDao.findMediaByContentIds`（IN 批量查询，替代逐条 findMedia，DB N+1→恒 2）+ 内存 `groupMediaByContent` 分组 + `buildContentMedia` 媒体损坏跳过逻辑原样），事务提交后 `rebuildRedis` 在**事务外**写 Redis；DB 失败记日志 return 不触发任何 Redis 写。内容 key 批量写走新增 `CacheAside.writeBatch`（一趟 pipeline `setex[per-key TTL 抖动]+del empty:×N`，失败→逐 key `deleteQuietly` 自愈 + WRITE_FAIL 打点，与 writeOrInvalidate 语义一致；批内单命令 server 错误依赖 `Pipeline.sync()` 抛异常兜底）；`rebuildIndexes` 单条 executeVoid——SCAN 顺序收集旧索引 key → 一趟 pipeline DEL 全部 + `lremAndLpush(Pipeline,...)` 全部（ensureIndex 懒重建/removeContent/addToIndex 零改动）。`addContent`/`refreshContent` 单写仍走 `writeOrInvalidate`。
  * **前后对比（验收"启动往返次数显著下降"）**：前 = DB N+1 + Redis ≈ **12N 往返**（内容 2 + 索引 8/内容）；后 = DB 恒 **2** + Redis ≈ **3**（内容 pipeline 1 + 索引 SCAN 页 + 索引 pipeline 1），与内容量解耦。运行时 `INFO commandstats` 复核可选（测试库数据量小、运行时刻度意义有限），以单测断言 `initRedisRoundTripsConstantForLargeContentCount`（executeVoid 恒 1/execute 恒 0）+ `initLoadsMediaInOneBatchQueryEliminatingNPlusOne` + `initMovesRedisWritesOutsideDbTransaction`（InOrder 断言事务先于缓存写）为验收证据。
  * **验证**：`tv.py test junit` **362 例全绿**（surefire 358 + 独立 fork pool-test 4；T4 末 356 → +8：ContentCacheTest init 区 2→5 例 + 媒体损坏跳过 +5、CacheAsideTest writeBatch +2——04c 后基数 356 含 pool 4，实际 surefire 355→358）+ `tv.py test all` **pytest 124 passed** 无回归；**独立 subagent 评审通过**（无🔴；🟡 4 条全部落实：媒体损坏跳过补测 initSkipsMediaBrokenContentButRebuildsIndexesForIntactOnes、writeBatch sync 兜底 javadoc、往返断言注释修正、NEEDS/TASKS/常青文档回写随 commit）。
  * **验收对照**：Redis 写入不在 DB 事务内 ✓（loadBuildableFromDb 无 Redis 调用 + InOrder 断言）；批量 pipeline 化 ✓（writeBatch 单趟 + rebuildIndexes 单趟，12N→3 前后对比）；R-01 拍板落地 ✓（全量+工程化优化，NEEDS R-01 已更新）；mvn compile + JUnit 362 全绿 ✓；pytest all 124 全绿 ✓。发现既有问题：无新增（U-11 为 T1 已登记）。
...
| 2026-09-14 | 0.7 | **T3 执行完成回写**（fix(cache-03)）：总览 T3 状态 草稿→已完成（2026-09-14）；T3 详情追加"执行回写"——探索结论（治理面=2 个 loader；区分机制用户拍板=行为保持，loader 失败抛 DatabaseException，CacheAside 捕获转 null）、实现摘要（ContentCache/CommentCache loader 重构 + addContent/refreshContent 守卫 + CacheAside 5 装载点 DatabaseException→null + loadDegraded helper 删 loadBatch）、验证结果（JUnit 348 全绿 +7、pytest all 124 passed、subagent 评审通过无🔴🟡 4 条二落实一记录一可接受） |
| 2026-09-14 | 0.8 | **T4 拆 commit 校准 + 04a 执行回写**（fix(cache-04a)）：总览 T4 commit 主题列改 `fix(cache-04a/04b/04c)`（用户拍板 T4 三项拆 3 commit：a=①N3 空标记守卫、b=②N4 索引自愈、c=③N7 条件写竞态；任务编号引用仍用 T4），状态 草稿→执行中（04a 完成）；T4 详情追加"执行回写（04a）"——探索结论（markEmpty 调用方 5 处全在单飞 leader 内、exists 守卫方案用户拍板对齐 writeSet 先例、del(dataKey) 随守卫移除=死代码+竞态危害源）、实现（markEmpty 存在守卫 + writeSet 空分支定向复用，残余竞态=exists→setex 毫秒间隙可自愈已接受）、验证（JUnit 351 全绿 +3 含并发不假空时序测试、pytest 124 passed）；测试适配 6 处断言随 del 移除与复用调整 |
| 2026-09-14 | 0.9 | **04b 执行回写**（fix(cache-04b)）：总览 T4 状态 → 执行中（04a/04b 完成）；T4 详情追加"执行回写（04b）"——探索结论（N4 缺陷链=写失败仅日志+ensureIndex 只判存在致懒重建永不触发；自愈基建已在、缺 DEL 触发器；方案=catch 内 best-effort DEL 4 个索引 key 复用懒重建，零新增 key，否决脏标记/完整性校验）、失败三分收敛（抖动已过=DEL 成功自愈/持续挂=双层 best-effort 不抛与现状一致/部分 DEL=照样触发全量重建）、实现（indexKeysOf 同源提取 + addToIndex catch 自愈 + deleteIndexKeysQuietly；ensureIndex/rebuildIndexes/读路径零改动；不引入 stats 挂点）、验证（JUnit 353 全绿 +2、pytest 124 passed；测试实测修正 doNothing 覆盖直通 stub 须 doAnswer）、残余窗口（持续挂恢复后索引仍可能不完整，与现状一致；触发面不扩大，不做 R-10） |
| 2026-09-14 | 1.0 | **04c 执行回写**（fix(cache-04c)，**T4 关闭**）：总览 T4 状态 → 已完成（04a/04b/04c 2026-09-14）；T4 详情追加"执行回写（04c）"——探索结论（治理面实为 4 方法，unlike 的 DECR 以 -1 重建同款竞态用户拍板同修；Jedis 5.1.0 eval javap 实证；executeVoid 统一包装 CacheException 熔断口径不变；comment 版写路径零测试缺口）、方案（LIKE/UNLIKE_CONDITIONAL_SCRIPT 两脚本常量，4 方法体统一 eval，零新增 key，unlike 保持不清空标记语义）、验证（JUnit 356 全绿 −1+2+2 失效降级对称覆盖、pytest 124；条件语义内聚脚本单测验证调用参数 + 运行时 Lua 冒烟 5 场景 10 断言全过 temp_script/verify_cache04c_lua.py）、验收对照（修掉 ✓ 非记录接受） |
| 2026-09-14 | 1.1 | **T5 执行完成回写**（fix(cache-05)）：总览 T5 状态 草稿→已完成（2026-09-14）；T5 详情追加"执行回写"——R-01 拍板（用户 2026-09-14 = 全量 + 工程化优化，理由记录）、探索结论（init 由 IocContainer invokeInitializable 调用、现状量化 DB N+1 + Redis ≈12N 往返全串一个长事务、三取向代价对比）、实现摘要（init 拆两段事务外写/批量媒体装载消 N+1/writeBatch 内容批量/rebuildIndexes pipeline）、前后对比（12N→3、N+1→2）、验证结果（JUnit 362 全绿 + pytest 124 + subagent 评审无🔴🟡4 条全落实）、验收对照四项全过 |
